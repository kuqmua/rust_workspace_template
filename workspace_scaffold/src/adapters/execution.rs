...
pub(crate) fn run_cargo(
    ...
    let mut command = macros_helpers::domain_types::tool_command::ToolCommand::new(
        macros_helpers::domain_types::tool_command::ToolProgramRef::from("cargo"),
    );
    let _arguments = command
        .current_dir(macros_helpers::domain_types::tool_command::PathRef::from(
            root.get(),
        ))
        .args(macros_helpers::domain_types::tool_command::ToolArgsRef::from(arguments.get()));
    if let Some(environment) = update_environment {
        let _environment = command.env(
            macros_helpers::domain_types::tool_command::ToolEnvKeyRef::from(environment.get()),
            macros_helpers::domain_types::tool_command::ToolEnvValueRef::from("1"),
        );
    }
