...
fn rejects_scaffold_text_over_size_limit() {
    ...
    assert!(
        matches!(
            result,
            Err(server_runtime_http::domain_types::BoundedReadError::ExceedsMaximum { .. })
        ),
        "8f32bc16"
    );
...
fn service_scaffold_registers_all_artifacts() {
    ...
    (|| -> Result<(), super::ScaffoldError> {
        let root_ref = super::ScaffoldPathRef::from(root.as_path());
        let service_name = super::ProjectNameRef::from(constants_str::VALUE_E896B9AF);
        let port = super::ServicePort::from(8082u16);
        super::naming_validate_project_name::naming_validate_project_name(service_name)?;
        if port.0 == constants_u16::ZERO {
            return Err(super::ScaffoldError::ServicePort);
        }
        ...
        if [service, config.as_str(), contract.as_str()]
            .iter()
            .any(|path| root_ref.0.join(path).exists())
        {
            return Err(super::ScaffoldError::ServiceExists);
        }
        let kebab = super::naming_kebab_case::naming_kebab_case(service_name);
        ...
        let replacements = [
            ...
            (
                ...
                super::naming_upper_camel_case::naming_upper_camel_case(service_name)
                    .as_ref()
                    .to_owned(),
            ),
            (
        ...
        crate::template_fs_copy_template_tree::template_fs_copy_template_tree(
            super::ScaffoldPathRef::from(
                root_ref.0
                    .join(constants_str::WORKSPACE_SCAFFOLD_NOTIFICATION_SERVICE)
                    .as_path(),
            ),
            super::ScaffoldPathRef::from(root_ref.0.join(service).as_path()),
        ...
        crate::template_fs_copy_template_tree::template_fs_copy_template_tree(
            ...
            super::ScaffoldPathRef::from(root_ref.0.join(config.as_str()).as_path()),
            super::ReplacementsRef::from(replacements.as_slice()),
        )?;
        crate::template_fs_copy_template_tree::template_fs_copy_template_tree(
            super::ScaffoldPathRef::from(
            ...
            super::ScaffoldPathRef::from(root_ref.0.join(contract.as_str()).as_path()),
            super::ReplacementsRef::from(replacements.as_slice()),
        )?;

        let manifest = root_ref.0.join(constants_str::CARGO_TOML);
        crate::template_fs_insert_once::template_fs_insert_once(
            super::ScaffoldPathRef::from(manifest.as_path()),
            super::ScaffoldTextRef::from(constants_str::WORKSPACE_SCAFFOLD_MANIFEST_MEMBER_MARKER),
            super::ScaffoldTextRef::from(
                format!(
        ...
        crate::template_fs_insert_once::template_fs_insert_once(
            super::ScaffoldPathRef::from(manifest.as_path()),
            super::ScaffoldTextRef::from(dependency_marker),
            super::ScaffoldTextRef::from(
                format!(
        ...
        crate::template_fs_replace_file::template_fs_replace_file(
            super::ScaffoldPathRef::from(k8s_destination.as_path()),
            super::ReplacementsRef::from(replacements.as_slice()),
        )?;
        ...
        crate::template_fs_write_text::template_fs_write_text(
            super::ScaffoldPathRef::from(k8s_destination.as_path()),
            super::ScaffoldTextRef::from(k8s_contents.as_str()),
        )?;
        let kustomization = root_ref
            .0
            .join(constants_str::WORKSPACE_SCAFFOLD_KUSTOMIZATION_PATH);
        crate::template_fs_insert_once::template_fs_insert_once(
            super::ScaffoldPathRef::from(kustomization.as_path()),
            super::ScaffoldTextRef::from(constants_str::WORKSPACE_SCAFFOLD_KUSTOMIZATION_MARKER),
            super::ScaffoldTextRef::from(
        ...
        let config_example = crate::template_fs_read_bounded_text::template_fs_read_bounded_text(
            super::ScaffoldPathRef::from(config_example_path.as_path()),
        )?;
        ...
        let compose_environment = config_example
            ...
            .lines()
            .map(|line| {
                let (key, example) = line.split_once('=').ok_or(super::ScaffoldError::Catalog)?;
            ...
            .collect::<Result<String, super::ScaffoldError>>()?;
        let ready_path =
        ...
        let compose_path = root_ref.0.join(format!("docker-compose.{service}.yml"));
        crate::template_fs_write_text::template_fs_write_text(
            super::ScaffoldPathRef::from(compose_path.as_path()),
            super::ScaffoldTextRef::from(compose.as_str()),
        )?;
        ...
        let mut service_catalog_contents =
            crate::template_fs_read_bounded_text::template_fs_read_bounded_text(
                super::ScaffoldPathRef::from(service_catalog.as_path()),
            )?
            .as_ref()
            .to_owned();
        ...
        crate::template_fs_write_text::template_fs_write_text(
            super::ScaffoldPathRef::from(service_catalog.as_path()),
            super::ScaffoldTextRef::from(service_catalog_contents.as_str()),
        )?;
        Ok(())
    })()
