...
impl TryFrom<&String> for StatusCode {
    ...
    fn try_from(v: &String) -> Result<Self, Self::Error> {
        if v == str_constants::CONTINUE_100 {
            Ok(Self::Continue100)
        } else if v == str_constants::SWITCHING_PROTOCOLS_101 {
            Ok(Self::SwitchingProtocols101)
        } else if v == str_constants::PROCESSING_102 {
            Ok(Self::Processing102)
        } else if v == str_constants::VALUE_200_OK {
            Ok(Self::Ok200)
        } else if v == str_constants::CREATED_201 {
            Ok(Self::Created201)
        } else if v == str_constants::ACCEPTED_202 {
            Ok(Self::Accepted202)
        } else if v == str_constants::NON_AUTHORITATIVE_INFORMATION_203 {
            Ok(Self::NonAuthoritativeInformation203)
        } else if v == str_constants::NO_CNT_204 {
            Ok(Self::NoContent204)
        } else if v == str_constants::RESET_CNT_205 {
            Ok(Self::ResetContent205)
        } else if v == str_constants::PARTIAL_CNT_206 {
            Ok(Self::PartialContent206)
        } else if v == str_constants::MULTI_STATUS_207 {
            Ok(Self::MultiStatus207)
        } else if v == str_constants::ALREADY_REPORTED_208 {
            Ok(Self::AlreadyReported208)
        } else if v == str_constants::IM_USED_226 {
            Ok(Self::ImUsed226)
        } else if v == str_constants::MULTIPLE_CHOICES_300 {
            Ok(Self::MultipleChoices300)
        } else if v == str_constants::MOVED_PERMANENTLY_301 {
            Ok(Self::MovedPermanently301)
        } else if v == str_constants::FOUND_302 {
            Ok(Self::Found302)
        } else if v == str_constants::SEE_OTHER_303 {
            Ok(Self::SeeOther303)
        } else if v == str_constants::NOT_MODIFIED_304 {
            Ok(Self::NotModified304)
        } else if v == str_constants::USE_PROXY_305 {
            Ok(Self::UseProxy305)
        } else if v == str_constants::TEMPORARY_REDIRECT_307 {
            Ok(Self::TemporaryRedirect307)
        } else if v == str_constants::PERMANENT_REDIRECT_308 {
            Ok(Self::PermanentRedirect308)
        } else if v == str_constants::BAD_REQ_400 {
            Ok(Self::BadReq400)
        } else if v == str_constants::UNAUTHORIZED_401 {
            Ok(Self::Unauthorized401)
        } else if v == str_constants::PAYMENT_REQUIRED_402 {
            Ok(Self::PaymentRequired402)
        } else if v == str_constants::FORBIDDEN_403 {
            Ok(Self::Forbidden403)
        } else if v == str_constants::NOT_FOUND_404 {
            Ok(Self::NotFound404)
        } else if v == str_constants::METHOD_NOT_ALLOWED_405 {
            Ok(Self::MethodNotAllowed405)
        } else if v == str_constants::NOT_ACCEPTABLE_406 {
            Ok(Self::NotAcceptable406)
        } else if v == str_constants::PROXY_AUTHENTICATION_REQUIRED_407 {
            Ok(Self::ProxyAuthenticationRequired407)
        } else if v == str_constants::REQ_TIMEOUT_408 {
            Ok(Self::ReqTimeout408)
        } else if v == str_constants::CONFLICT_409 {
            Ok(Self::Conflict409)
        } else if v == str_constants::GONE_410 {
            Ok(Self::Gone410)
        } else if v == str_constants::LENGTH_REQUIRED_411 {
            Ok(Self::LengthRequired411)
        } else if v == str_constants::PRECONDITION_FAILED_412 {
            Ok(Self::PreconditionFailed412)
        } else if v == str_constants::PAYLOAD_TOO_LARGE_413 {
            Ok(Self::PayloadTooLarge413)
        } else if v == str_constants::URI_TOO_LONG_414 {
            Ok(Self::UriTooLong414)
        } else if v == str_constants::UNSUPPORTED_MEDIA_TYPE_415 {
            Ok(Self::UnsupportedMediaType415)
        } else if v == str_constants::RANGE_NOT_SATISFIABLE_416 {
            Ok(Self::RangeNotSatisfiable416)
        } else if v == str_constants::EXPECTATION_FAILED_417 {
            Ok(Self::ExpectationFailed417)
        } else if v == str_constants::IM_A_TEAPOT_418 {
            Ok(Self::ImATeapot418)
        } else if v == str_constants::MISDIRECTED_REQ_421 {
            Ok(Self::MisdirectedReq421)
        } else if v == str_constants::UNPROCESSABLE_ENTITY_422 {
            Ok(Self::UnprocessableEntity422)
        } else if v == str_constants::LOCKED_423 {
            Ok(Self::Locked423)
        } else if v == str_constants::FAILED_DEPENDENCY_424 {
            Ok(Self::FailedDependency424)
        } else if v == str_constants::UPGRADE_REQUIRED_426 {
            Ok(Self::UpgradeRequired426)
        } else if v == str_constants::PRECONDITION_REQUIRED_428 {
            Ok(Self::PreconditionRequired428)
        } else if v == str_constants::TOO_MANY_REQS_429 {
            Ok(Self::TooManyReqs429)
        } else if v == str_constants::REQ_HEADER_FIELDS_TOO_LARGE_431 {
            Ok(Self::ReqHeaderFieldsTooLarge431)
        } else if v == str_constants::UNAVAILABLE_FOR_LEGAL_REASONS_451 {
            Ok(Self::UnavailableForLegalReasons451)
        } else if v == str_constants::INTERNAL_SERVER_ERROR_500 {
            Ok(Self::InternalServerError500)
        } else if v == str_constants::NOT_IMPLEMENTED_501 {
            Ok(Self::NotImplemented501)
        } else if v == str_constants::BAD_GATEWAY_502 {
            Ok(Self::BadGateway502)
        } else if v == str_constants::SERVICE_UNAVAILABLE_503 {
            Ok(Self::ServiceUnavailable503)
        } else if v == str_constants::GATEWAY_TIMEOUT_504 {
            Ok(Self::GatewayTimeout504)
        } else if v == str_constants::HTTP_VERSION_NOT_SUPPORTED_505 {
            Ok(Self::HttpVersionNotSupported505)
        } else if v == str_constants::VARIANT_ALSO_NEGOTIATES_506 {
            Ok(Self::VariantAlsoNegotiates506)
        } else if v == str_constants::INSUFFICIENT_STORAGE_507 {
            Ok(Self::InsufficientStorage507)
        } else if v == str_constants::LOOP_DETECTED_508 {
            Ok(Self::LoopDetected508)
        } else if v == str_constants::NOT_EXTENDED_510 {
            Ok(Self::NotExtended510)
        } else if v == str_constants::NETWORK_AUTHENTICATION_REQUIRED_511 {
