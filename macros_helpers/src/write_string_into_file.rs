    ...
    match std::fs::metadata(path_ref) {
        Ok(v) => {
            let new_len_u64 = u64::try_from(string_cnt_ref.len()).map_err(|_error| {
                std::io::Error::other(
                    str_constants::text::VALUE_2F4D7A8C_FAILED_CONVERTING_STRING_LENGTH,
                )
            })?;
            ...
            loop {
                ...
                let end = offset
                    ...
                    .ok_or_else(|| std::io::Error::other(str_constants::text::VALUE_5F28D14C_GENERATED_FILE_COMPARISON_OFFSET_OVERFLOW))?;
                let Some(new_chunk) = string_cnt_ref.as_bytes().get(offset..end) else {
                    let _validated_old_cnt = std::fs::read_to_string(path_ref)?;
                    return Ok(ShouldWriteString::from(true));
                };
                let Some(old_chunk_read) = old_chunk.get(..read_len) else {
                    return Err(std::io::Error::other(str_constants::text::F83D470A_GENERATED_FILE_COMPARISON_READ_LENGTH_EXCEEDS_BUFFER));
                };
...
fn should_write_with_same_len_diff(
    ...
    if old.len() != new.len() {
        return Err(std::io::Error::other(
            str_constants::text::F4C1D7A9_SAME_LEN_HELPER_REQUIRES_EQUAL_LENGTHS,
        ));
    }
...
fn should_write_with_diff_len(
    ...
    if old.len() == new.len() {
        return Err(std::io::Error::other(
            str_constants::text::VALUE_9A6D2C1B_DIFF_LEN_HELPER_REQUIRES_DIFFERENT_LENGTHS,
        ));
    }
    std::fs::write(path, old)?;
    should_write_string_into_file(
...
mod tests {
    ...
    fn txt_path(name: &str) -> std::path::PathBuf {
        crate::test_hlp::test_path(crate::test_hlp::TestPathStem::new(name))
            .0
            .with_extension(str_constants::text::TXT)
    }
    ...
    fn try_write_string_into_path_writes_exact_content() {
        let path = txt_path(str_constants::text::MACROS_HELPERS_WRITE_PATH);
        let result_path =
            super::try_write_string_into_path(&path, cnt(str_constants::text::ABC_ALT_3))
                .expect("dcb22948");
        assert_eq!(result_path, written_path(path.clone()));
        assert_content_and_cleanup(path.as_path(), str_constants::text::ABC_ALT_3);
    }
    #[test]
    fn write_string_into_file_adds_rs_extension() {
        let base = crate::test_hlp::test_path(crate::test_hlp::TestPathStem::new(
            str_constants::text::MACROS_HELPERS_WRITE_FILE,
        ));
        let path = crate::rs_file_path::rs_file_path(&base);
        super::write_string_into_file(&base, cnt(str_constants::text::XYZ));
        assert_content_and_cleanup(path.0.as_path(), str_constants::text::XYZ);
    }
    #[test]
    fn try_write_string_into_file_returns_path() {
        let base = crate::test_hlp::test_path(crate::test_hlp::TestPathStem::new(
            str_constants::text::MACROS_HELPERS_TRY_WRITE_FILE,
        ));
        let path = super::try_write_string_into_file(&base, cnt(str_constants::text::QWE))
            .expect("6676e082");
        assert_content_and_cleanup(path.as_ref(), str_constants::text::QWE);
    }
    #[test]
    fn try_write_string_into_path_writes_exact_path_without_extension_rewrite() {
        let path = txt_path(str_constants::text::MACROS_HELPERS_TRY_WRITE_PATH_PASSTHROUGH);
        let result_path =
            super::try_write_string_into_path(&path, cnt(str_constants::text::ABC_ALT_3))
                .expect("b6b47a2c");
        assert_eq!(result_path, written_path(path.clone()));
    ...
    fn should_write_string_into_file_returns_true_for_missing_file() {
        let path = txt_path(str_constants::text::MACROS_HELPERS_SHOULD_WRITE_MISSING);
        let should_write = super::should_write_string_into_file(
            path_ref(&path),
            cnt(str_constants::text::ABC_ALT_3),
        )
        .expect("f5d2cb68");
    ...
    fn should_write_string_into_file_returns_false_when_content_is_eq() {
        let path = txt_path(str_constants::text::MACROS_HELPERS_SHOULD_WRITE_SAME);
        std::fs::write(&path, str_constants::text::SAME).expect("68e4f52d");
        let should_write =
            super::should_write_string_into_file(path_ref(&path), cnt(str_constants::text::SAME))
                .expect("3e7adf2f");
        assert!(!bool::from(should_write));
    ...
    fn should_write_string_into_file_compares_equal_content_in_chunks() {
        let path = txt_path(str_constants::text::MACROS_HELPERS_SHOULD_WRITE_LARGE_SAME);
        let content = str_constants::text::ABCD_ALT.repeat(4097usize);
    ...
    fn should_write_string_into_file_finds_diff_after_first_chunk() {
        let path = txt_path(str_constants::text::MACROS_HELPERS_SHOULD_WRITE_LARGE_DIFF);
        let old_content = str_constants::text::A_ALT.repeat(16_388usize);
        let mut new_content = old_content.clone();
        new_content.replace_range(16_387usize.., str_constants::text::B);
    ...
    fn should_write_string_into_file_returns_true_when_content_differs() {
        let path = txt_path(str_constants::text::MACROS_HELPERS_SHOULD_WRITE_DIFF);
        std::fs::write(&path, str_constants::text::OLD).expect("a2fd8473");
        let should_write =
            super::should_write_string_into_file(path_ref(&path), cnt(str_constants::text::NEW))
                .expect("52c9a1db");
        assert!(bool::from(should_write));
        cleanup(path.as_path());
    }
    ...
    fn should_write_string_into_file_returns_true_for_same_len_diff_content() {
        let path = txt_path(str_constants::text::MACROS_HELPERS_SHOULD_WRITE_SAME_LEN_DIFF);
        let should_write = super::should_write_with_same_len_diff(
            &path,
            str_constants::text::ABC_ALT_3,
            str_constants::text::XYZ,
        )
        .expect("517fd0c9");
        assert!(bool::from(should_write));
        cleanup(path.as_path());
    ...
    fn should_write_string_into_file_returns_true_for_diff_len_content() {
        let path = txt_path(str_constants::text::MACROS_HELPERS_SHOULD_WRITE_DIFF_LEN);
        let should_write = super::should_write_with_diff_len(
            &path,
            str_constants::text::ABCD_ALT,
            str_constants::text::A_ALT,
        )
        .expect("e2d99b73");
        assert!(bool::from(should_write));
        cleanup(path.as_path());
    ...
    fn write_string_if_needed_returns_false_without_rewrite_for_eq_content() {
        let path = txt_path(str_constants::text::MACROS_HELPERS_WRITE_IF_NEEDED_EQ);
        std::fs::write(&path, str_constants::text::SAME).expect("924bdc58");
        let wrote = super::write_string_if_needed(path_ref(&path), cnt(str_constants::text::SAME))
            .expect("9f27b9cb");
        assert!(!bool::from(wrote));
        assert_content_and_cleanup(path.as_path(), str_constants::text::SAME);
    }
    #[test]
    fn write_string_if_needed_returns_true_and_writes_for_diff_content() {
        let path = txt_path(str_constants::text::MACROS_HELPERS_WRITE_IF_NEEDED_DIFF);
        std::fs::write(&path, str_constants::text::OLD).expect("9b4ab8ad");
        let wrote = super::write_string_if_needed(path_ref(&path), cnt(str_constants::text::NEW))
            .expect("4e4ce16d");
        assert!(bool::from(wrote));
        assert_content_and_cleanup(path.as_path(), str_constants::text::NEW);
    }
    #[test]
    fn path_with_rs_extension_accepts_path_input() {
        let path = crate::rs_file_path::rs_file_path(crate::test_hlp::test_path(
            crate::test_hlp::TestPathStem::new(str_constants::text::MACROS_HELPERS_RS_EXT_PATH),
        ));
    ...
    fn try_write_string_into_file_skips_rewrite_when_cnt_is_unchanged() {
        let base = crate::test_hlp::test_path(crate::test_hlp::TestPathStem::new(
            str_constants::text::MACROS_HELPERS_WRITE_IF_CHANGED,
        ));
        let path = crate::rs_file_path::rs_file_path(&base);
        std::fs::write(&path, str_constants::text::SAME).expect("0242e1a9");
        let metadata_before = std::fs::metadata(&path).expect("974bc327");
        let _path = super::try_write_string_into_file(&base, cnt(str_constants::text::SAME))
            .expect("07d9fd90");
        let metadata_after = std::fs::metadata(&path).expect("83087942");
        assert_eq!(metadata_before.len(), metadata_after.len());
        assert_content_and_cleanup(path.0.as_path(), str_constants::text::SAME);
    }
    #[test]
    fn try_write_string_into_file_writes_when_cnt_differs() {
        let base = crate::test_hlp::test_path(crate::test_hlp::TestPathStem::new(
            str_constants::text::MACROS_HELPERS_WRITE_IF_CHANGED_DIFF,
        ));
        let path = crate::rs_file_path::rs_file_path(&base);
        std::fs::write(&path, str_constants::text::OLD).expect("d870b82e");
        let _path = super::try_write_string_into_file(&base, cnt(str_constants::text::NEW))
            .expect("c6fd2bc8");
        assert_content_and_cleanup(path.0.as_path(), str_constants::text::NEW);
    }
    #[test]
    fn try_write_string_into_path_with_outcome_returns_changed_for_new_content() {
        let path = txt_path(str_constants::text::MACROS_HELPERS_WRITE_OUTCOME_CHANGED);
        let outcome = super::try_write_string_into_path_with_outcome(
            &path,
            cnt(str_constants::text::ABC_ALT_3),
        )
        .expect("947faed1");
        crate::test_hlp::assert_file_content(
            crate::test_hlp::StdAssertFilePath::new(&path),
            crate::test_hlp::ExpectedFileContent::new(str_constants::text::ABC_ALT_3),
        );
    ...
    fn try_write_string_into_path_with_outcome_returns_unchanged_for_same_content() {
        let path = txt_path(str_constants::text::MACROS_HELPERS_WRITE_OUTCOME_UNCHANGED);
        std::fs::write(&path, str_constants::text::ABC_ALT_3).expect("d293f783");
        let outcome = super::try_write_string_into_path_with_outcome(
            &path,
            cnt(str_constants::text::ABC_ALT_3),
        )
    ...
    fn try_write_string_into_file_with_outcome_returns_changed_and_rs_path() {
        let base = crate::test_hlp::test_path(crate::test_hlp::TestPathStem::new(
            str_constants::text::MACROS_HELPERS_WRITE_FILE_OUTCOME_CHANGED,
        ));
        let path = crate::rs_file_path::rs_file_path(&base);
        let outcome = super::try_write_string_into_file_with_outcome(
            &base,
            cnt(str_constants::text::ABC_ALT_3),
        )
        .expect("57cf209a");
        ...
        assert_content_and_cleanup(path.0.as_path(), str_constants::text::ABC_ALT_3);
    }
    #[test]
    fn try_write_string_into_file_with_outcome_returns_unchanged_for_same_content() {
        let base = crate::test_hlp::test_path(crate::test_hlp::TestPathStem::new(
            str_constants::text::MACROS_HELPERS_WRITE_FILE_OUTCOME_UNCHANGED,
        ));
        let path = crate::rs_file_path::rs_file_path(&base);
        std::fs::write(&path, str_constants::text::ABC_ALT_3).expect("2199f0a7");
        let outcome = super::try_write_string_into_file_with_outcome(
            &base,
            cnt(str_constants::text::ABC_ALT_3),
        )
    ...
    fn write_path_outcome_into_path_returns_owned_path() {
        let changed_path =
            txt_path(str_constants::text::MACROS_HELPERS_WRITE_OUTCOME_INTO_PATH_CHANGED);
        ...
        let unchanged_path =
            txt_path(str_constants::text::MACROS_HELPERS_WRITE_OUTCOME_INTO_PATH_UNCHANGED);
