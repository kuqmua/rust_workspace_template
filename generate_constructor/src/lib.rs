...
#[allow(
    ...
    reason = "fallible parsing stays outside the infallible proc-macro entry point"
)]
fn try_generate_new(input: proc_macro::TokenStream) -> syn::Result<proc_macro2::TokenStream> {
    let parsed_input = syn::parse::<syn::DeriveInput>(input)?;
    generate_new_token_stream(&parsed_input)
}

#[allow(
    clippy::single_call_fn,
    reason = "token generation is separated from the proc-macro boundary"
)]
fn generate_new_token_stream(input: &syn::DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    ...
    let visibility = input
        .attrs
        .iter()
        .try_fold(input.vis.clone(), |visibility, attribute| {
            if attribute
                .path()
