...
Всего структур-обёрток: **935**. **I:** 935 Да / 0 Нет. **D:** 97 Да / 838 Нет. **B:** 271 Да / 664 Нет. **DT:** 58 Да / 877 Нет. **FT:** 92 Да / 843 Нет.

Проверено в коде: **935** типов; исправлено или уже было корректно ограничено: **192**; изменений по ограничению размера не требуется: **743**; типов со статусом **Не проверено** нет. Столбец **Статус** относится к ограничению размера (`B`/`DT`/`FT`), а не к соблюдению правила **I**.

## Итог аудита инициализации

Все **935 из 935** объявленных в исходниках wrapper-структур теперь создаются только через `From`/`TryFrom`: внутренние tuple-поля закрыты, прямые вызовы `Type(value)` и `Self(value)` вне conversion-impl запрещены code-style тестом.
