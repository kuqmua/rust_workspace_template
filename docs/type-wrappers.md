...
Всего структур-обёрток: **937**. **I:** 937 Да / 0 Нет. **D:** 98 Да / 839 Нет. **B:** 272 Да / 665 Нет. **DT:** 59 Да / 878 Нет. **FT:** 93 Да / 844 Нет.

Проверено в коде: **937** типов; исправлено или уже было корректно ограничено: **193**; изменений по ограничению размера не требуется: **744**; типов со статусом **Не проверено** нет. Столбец **Статус** относится к ограничению размера (`B`/`DT`/`FT`), а не к соблюдению правила **I**.

## Итог аудита инициализации

Все **937 из 937** объявленных в исходниках wrapper-структур теперь создаются только через `From`/`TryFrom`: внутренние tuple-поля закрыты, прямые вызовы `Type(value)` и `Self(value)` вне conversion-impl запрещены code-style тестом.
