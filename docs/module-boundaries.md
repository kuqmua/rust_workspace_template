...
| `dev_data_bootstrap` | Development-only seed command | One command boundary; not linked into service startup. |
...
| `pg_crud_macro_common`, `pg_crud_macro_common_macros` | Shared proc-macro parsing and generation helpers. |
...
| `prepare_pg_databases` | Database preparation command; no application queries. |
...
| `workspace_macro_helpers`, `macro_helpers`, `generate_derive_token_stream_builder`, `generate_quotes`, `macro_clippy_check_common`, `optimal_memory_layout` | Compile-time support crates; none owns service runtime behavior. |
...
| `init_env_files` | Environment-file initialization command | One command boundary. |
