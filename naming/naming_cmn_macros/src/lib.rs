...
pub fn case_trait_pair(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let Some(bound_ts) = workspace_macro_helpers::part_at(&parts, 2) else {
        return workspace_macro_helpers::compile_error_ts("case_trait_pair expects bound")
            .into_inner()
            .into();
    };
    let Some(closure_text) =
        ...
        return workspace_macro_helpers::compile_error_ts("case_trait_pair expects closure")
            .into_inner()
            .into();
    };
    ...
    quote::quote! {
        ...
        {
            fn case_or_panic(&self) -> proc_macro2::TokenStream {
                to_ts_or_panic(&#str_trait_ident::case(self)).0
            }
        }
    }
    .into()
