...
pub fn errors_with_location(
    ...
    if !attr_token_stream.is_empty() {
        return syn::Error::new(proc_macro2::Span::call_site(), str_constants::expr::S_1254)
            .into_compile_error()
            .into();
    }
    ...
    item_ref.variants.iter_mut().try_for_each(|variant| {
        ...
        if fields.named.iter().any(|field| {
            field
                .ident
                .as_ref()
                .is_some_and(|identifier| identifier == str_constants::expr::S_1463)
        }) {
            return Err(syn::Error::new_spanned(
                variant,
                str_constants::expr::S_1256,
            ));
        }
        fields
            .named
...
pub fn location(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let utoipa_to_schema_token_stream = di
        .attrs
        .iter()
        .any(|attr| attr.path().is_ident(str_constants::expr::S_1466))
        ...
        SuportedEnumVariant::Named => {
                    ...
                    .map(|el0| {
                        ...
                        match macros_helpers::location_data::LocationFieldAttr::try_from(el0).expect("8ff56aeb") {
                            ...
                            macros_helpers::location_data::LocationFieldAttr::EoLocation => {
                                let if_write_is_err_token_stream = macros_helpers::generate_if_write_is_err_token_stream::generate_if_write_is_err_token_stream(&quote::quote! {accumulator_52e70d22, "\n {element}"}, &quote::quote! {panic!("c751d54a");});
                                quote::quote! {
                            ...
                            macros_helpers::location_data::LocationFieldAttr::EoVecToErrString | macros_helpers::location_data::LocationFieldAttr::EoVecToErrStringSerde => {
                                let if_write_is_err_token_stream = macros_helpers::generate_if_write_is_err_token_stream::generate_if_write_is_err_token_stream(&quote::quote! {accumulator_a9ba7521, "\n {element_6e4f53ad}"}, &quote::quote! {panic!("b35ed9f5");});
                                quote::quote! {
                            ...
                            macros_helpers::location_data::LocationFieldAttr::EoVecLocation => {
                                let if_write_is_err_token_stream = macros_helpers::generate_if_write_is_err_token_stream::generate_if_write_is_err_token_stream(&quote::quote! {accumulator_1bbd5ef3, "\n {element_3f2fe01d}"}, &quote::quote! {panic!("4dfdd18d");});
                                quote::quote! {
                                    #el0_identifier.iter().fold(
                                        #string_token_stream::new(),
                            ...
                            macros_helpers::location_data::LocationFieldAttr::EoHashMapKStringVLocation => {
                                let if_write_is_err_token_stream = macros_helpers::generate_if_write_is_err_token_stream::generate_if_write_is_err_token_stream(
                                    &{
                                        let if_write_is_err_token_stream = macros_helpers::generate_if_write_is_err_token_stream::generate_if_write_is_err_token_stream(&quote::quote! {accumulator_addfc699, "\n  {element_8b8f577e}"}, &quote::quote! {panic!("d0492fbf");});
                                        quote::quote! {
                    ...
                    .map(|el0| {
                        ...
                        if **el0_identifier == *location_snake_case_str {
                            ...
                            match macros_helpers::location_data::LocationFieldAttr::try_from(el0).expect("449c3781") {
                                ...
                                macros_helpers::location_data::LocationFieldAttr::EoLocation => generate_field_token_stream(&quote::quote! {
            ...
            let enum_identifier_with_serde_token_stream = {
                let vrts_token_stream = data_enum.variants.iter().map(|variant| {
                    macros_helpers::location_data::generate_serde_version_of_named_syn_variant(
                        macros_helpers::location_data::SynVariantRef::from(variant),
                    )
                });
...
mod tests {
    ...
    fn rejects_existing_location_field() {
        ...
        let error = super::add_location_fields(super::SynItemEnumMutRef::from(&mut item))
            .expect_err(str_constants::expr::S_0282);
        assert_eq!(
            error.to_string(),
    ...
    fn rejects_unnamed_variant() {
        ...
        let error = super::add_location_fields(super::SynItemEnumMutRef::from(&mut item))
            .expect_err(str_constants::expr::S_0546);
        assert_eq!(
            error.to_string(),
