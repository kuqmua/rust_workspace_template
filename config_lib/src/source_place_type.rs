...
impl SourcePlaceType {
    ...
    pub fn from_env_or_default() -> Self {
        ...
        let parsed = crate::env_var_result_var_error::EnvVarResultVarError::try_from(
            std::env::var(constants_str::ENV_NAMES_SOURCE_PLACE_TYPE),
        )
        ...
        .and_then(|env_v| {
            ...
            raw_v
                ...
                .map_err(|error| crate::env_parse_error::EnvParseError::Parse {
                    context: crate::parse_context_ref::ParseContextRef::from(
        ...
        match parsed {
            ...
            Err(message) => {
                tracing::warn!(
                    error = %message,
                    default = ?default,
                    fix = constants_str::CONFIG_SOURCE_PLACE_TYPE_FIX_MSG,
    ...
    #[cfg(test)]
    pub(super) fn parse_source_place_type_from_env_var(
