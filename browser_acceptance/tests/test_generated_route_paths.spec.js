...
test("test_generated_read_many_replaces_single_record_reads", async ({ page }) => {
  ...
  try {
    ...
    await resources.reduce(async (previous, resource) => {
      ...
      await ["read_one", "read_one_payload_example"].reduce(async (pending, operation) => {
        await pending;
        expect(document.paths[`/${resource}/${operation}`]).toBeUndefined();
        const removed = await page.request.fetch(`/${resource}/${operation}`, {
          method: operation === "read_one" ? "POST" : "GET",
          maxRedirects: 0,
        });
        expect(removed.status()).not.toBe(200);
      }, Promise.resolve());
      const legacy = await page.request.get(`/admin_${resource}/read_many_payload_example`, { maxRedirects: 0 });
