...
str_constants_macros::define_str_constants! {
    ...
    constants {
        ...
        pub SERVER_ADMIN_DASHBOARD_COUNTS_SQL = ["SELECT (SELECT COUNT(*) FROM admin_access_sessions WHERE revoked_at IS NULL AND expires_at > NOW())::BIGINT, (SELECT COUNT(*) FROM admin_login_attempts WHERE succeeded = FALSE AND attempted_at >= NOW() - INTERVAL '24 hours')::BIGINT"];
        pub SERVER_ADMIN_MFA_STATUS_SQL = ["SELECT EXISTS (SELECT 1 FROM admin_user_mfa WHERE user_id = $1 AND enabled_at IS NOT NULL), (SELECT COUNT(*)::BIGINT FROM admin_mfa_recovery_codes WHERE user_id = $1 AND used_at IS NULL)"];
        pub SERVER_ADMIN_UPSERT_MFA_SQL = ["INSERT INTO admin_user_mfa (user_id, encrypted_secret, secret_nonce, pending_created_at, enabled_at, updated_at) VALUES ($1, $2, $3, NOW(), NULL, NOW()) ON CONFLICT (user_id) DO UPDATE SET encrypted_secret = EXCLUDED.encrypted_secret, secret_nonce = EXCLUDED.secret_nonce, pending_created_at = NOW(), enabled_at = NULL, updated_at = NOW()"];
        pub SERVER_ADMIN_READ_MFA_SQL = ["SELECT encrypted_secret, secret_nonce, enabled_at IS NOT NULL FROM admin_user_mfa WHERE user_id = $1"];
        pub SERVER_ADMIN_ENABLE_MFA_SQL = ["UPDATE admin_user_mfa SET enabled_at = NOW(), updated_at = NOW() WHERE user_id = $1 AND enabled_at IS NULL RETURNING TRUE"];
        pub SERVER_ADMIN_DELETE_MFA_RECOVERY_SQL = ["DELETE FROM admin_mfa_recovery_codes WHERE user_id = $1"];
        pub SERVER_ADMIN_INSERT_MFA_RECOVERY_SQL = ["INSERT INTO admin_mfa_recovery_codes (user_id, code_hash) SELECT $1, value FROM UNNEST($2::TEXT[]) AS value"];
        pub SERVER_ADMIN_USE_MFA_RECOVERY_SQL = ["UPDATE admin_mfa_recovery_codes SET used_at = NOW() WHERE id = (SELECT id FROM admin_mfa_recovery_codes WHERE user_id = $1 AND code_hash = $2 AND used_at IS NULL ORDER BY id LIMIT 1 FOR UPDATE SKIP LOCKED) RETURNING TRUE"];
        pub SERVER_ADMIN_DELETE_MFA_SQL = ["DELETE FROM admin_user_mfa WHERE user_id = $1 RETURNING TRUE"];
        pub SERVER_ADMIN_MARK_MFA_STEP_UP_SQL = ["UPDATE admin_access_sessions SET mfa_verified_at = NOW() WHERE id = $1 AND user_id = $2 AND revoked_at IS NULL AND expires_at > NOW() RETURNING TRUE"];
...
pub const SERVER_ADMIN_COUNT_FILTERED_USERS_SQL: &str = "SELECT count(*) FROM admin_users WHERE ($1 = '' OR login ILIKE '%' || $1 || '%' OR display_name ILIKE '%' || $1 || '%' OR id::text = $1)";
pub const SERVER_ADMIN_PAGE_USERS_SQL: &str = "SELECT id, login, display_name, is_banned FROM admin_users WHERE ($1 = '' OR login ILIKE '%' || $1 || '%' OR display_name ILIKE '%' || $1 || '%' OR id::text = $1) ORDER BY CASE WHEN $2 = 'login' AND $3 = 'asc' THEN login END ASC, CASE WHEN $2 = 'login' AND $3 = 'desc' THEN login END DESC, CASE WHEN $2 = 'display_name' AND $3 = 'asc' THEN display_name END ASC, CASE WHEN $2 = 'display_name' AND $3 = 'desc' THEN display_name END DESC, CASE WHEN $2 = 'id' AND $3 = 'asc' THEN id END ASC, CASE WHEN $2 = 'id' AND $3 = 'desc' THEN id END DESC, CASE WHEN $2 = 'status' AND $3 = 'asc' THEN is_banned END ASC, CASE WHEN $2 = 'status' AND $3 = 'desc' THEN is_banned END DESC, id ASC LIMIT $4 OFFSET $5";
pub const SERVER_ADMIN_COUNT_FILTERED_ROLES_SQL: &str = "SELECT count(*) FROM admin_roles WHERE ($1 = '' OR name ILIKE '%' || $1 || '%' OR id::text = $1)";
pub const SERVER_ADMIN_PAGE_ROLES_SQL: &str = "SELECT id, name, is_system FROM admin_roles WHERE ($1 = '' OR name ILIKE '%' || $1 || '%' OR id::text = $1) ORDER BY CASE WHEN $2 = 'name' AND $3 = 'asc' THEN name END ASC, CASE WHEN $2 = 'name' AND $3 = 'desc' THEN name END DESC, CASE WHEN $2 = 'id' AND $3 = 'asc' THEN id END ASC, CASE WHEN $2 = 'id' AND $3 = 'desc' THEN id END DESC, CASE WHEN $2 = 'system' AND $3 = 'asc' THEN is_system END ASC, CASE WHEN $2 = 'system' AND $3 = 'desc' THEN is_system END DESC, id ASC LIMIT $4 OFFSET $5";
pub const SERVER_ADMIN_COUNT_FILTERED_PERMISSIONS_SQL: &str = "SELECT count(*) FROM admin_permissions WHERE ($1 = '' OR name ILIKE '%' || $1 || '%' OR id::text = $1)";
pub const SERVER_ADMIN_PAGE_PERMISSIONS_SQL: &str = "SELECT id, name FROM admin_permissions WHERE ($1 = '' OR name ILIKE '%' || $1 || '%' OR id::text = $1) ORDER BY CASE WHEN $2 = 'name' AND $3 = 'asc' THEN name END ASC, CASE WHEN $2 = 'name' AND $3 = 'desc' THEN name END DESC, CASE WHEN $2 = 'id' AND $3 = 'asc' THEN id END ASC, CASE WHEN $2 = 'id' AND $3 = 'desc' THEN id END DESC, id ASC LIMIT $4 OFFSET $5";
pub const SERVER_ADMIN_PAGE_AUDIT_LOG_SQL: &str = "SELECT id, user_id, user_login, action, resource, resource_id, succeeded, details, created_at::text FROM admin_audit_log WHERE ($1::bigint IS NULL OR user_id = $1) AND ($2::text IS NULL OR action = $2) AND ($3::text IS NULL OR resource = $3) AND ($4::timestamptz IS NULL OR created_at >= $4::timestamptz) AND ($5::timestamptz IS NULL OR created_at <= $5::timestamptz) AND ($6::timestamptz IS NULL OR (created_at, id) < ($6::timestamptz, $7::bigint)) AND ($8::text IS NULL OR user_login ILIKE '%' || $8 || '%') AND ($9::text IS NULL OR resource_id = $9) AND ($10::boolean IS NULL OR succeeded = $10) ORDER BY created_at DESC, id DESC LIMIT $11";
pub const SERVER_ADMIN_READ_PASSWORD_HASH_SQL: &str =
    "SELECT password_hash FROM admin_users WHERE id = $1 AND is_banned = FALSE";
pub const SERVER_ADMIN_REVOKE_OTHER_ACCESS_SESSIONS_SQL: &str = "UPDATE admin_access_sessions SET revoked_at = now() WHERE user_id = $1 AND id <> $2 AND revoked_at IS NULL";
