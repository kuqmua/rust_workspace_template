...
str_constants_macros::define_str_constants! {
    ...
    constants {
        ...
        pub SERVER_ADMIN_DATA_ACCESS_SESSIONS_SQL = [WORD_SELECT, " ARRAY[id::text, user_id::text, expires_at::text, created_at::text, revoked_at::text, mfa_verified_at::text] ", WORD_FROM, " ", WORD_ADMIN_ACCESS_SESSIONS, " ORDER BY created_at DESC, id DESC LIMIT 100"];
        pub SERVER_ADMIN_DATA_AUDIT_LOG_SQL = [WORD_SELECT, " ARRAY[id::text, user_id::text, left(user_login, 8192), left(action, 8192), left(resource, 8192), left(resource_id, 8192), request_id::text, succeeded::text, left(details::text, 8192), created_at::text] ", WORD_FROM, " ", WORD_ADMIN_AUDIT_LOG, " ORDER BY created_at DESC, id DESC LIMIT 100"];
        pub SERVER_ADMIN_DATA_CLEANUP_STATUS_SQL = [WORD_SELECT, " ARRAY[singleton::text, last_success_at::text, last_deleted_rows::text] ", WORD_FROM, " ", WORD_ADMIN_CLEANUP_STATUS, " ORDER BY singleton LIMIT 100"];
        pub SERVER_ADMIN_DATA_LOGIN_ATTEMPTS_SQL = [WORD_SELECT, " ARRAY[id::text, left(login, 8192), ip_address::text, succeeded::text, attempted_at::text] ", WORD_FROM, " ", WORD_ADMIN_LOGIN_ATTEMPTS, " ORDER BY attempted_at DESC, id DESC LIMIT 100"];
        pub SERVER_ADMIN_DATA_MFA_RECOVERY_CODES_SQL = [WORD_SELECT, " ARRAY[id::text, user_id::text, created_at::text, used_at::text] ", WORD_FROM, " ", WORD_ADMIN_MFA_RECOVERY_CODES, " ORDER BY created_at DESC, id DESC LIMIT 100"];
        pub SERVER_ADMIN_DATA_PERMISSIONS_SQL = [WORD_SELECT, " ARRAY[id::text, name, created_at::text] ", WORD_FROM, " ", WORD_ADMIN_PERMISSIONS, " ORDER BY id LIMIT 100"];
        pub SERVER_ADMIN_DATA_RATE_LIMITS_SQL = [WORD_SELECT, " ARRAY[left(scope, 8192), left(subject, 8192), window_started_at::text, request_count::text] ", WORD_FROM, " ", WORD_ADMIN_RATE_LIMITS, " ORDER BY window_started_at DESC, scope, subject LIMIT 100"];
        pub SERVER_ADMIN_DATA_REFRESH_TOKENS_SQL = [WORD_SELECT, " ARRAY[id::text, user_id::text, expires_at::text, created_at::text, revoked_at::text] ", WORD_FROM, " ", WORD_ADMIN_REFRESH_TOKENS, " ORDER BY created_at DESC, id DESC LIMIT 100"];
        pub SERVER_ADMIN_DATA_ROLE_PERMISSIONS_SQL = [WORD_SELECT, " ARRAY[id::text, role_id::text, permission_id::text, created_at::text] ", WORD_FROM, " ", WORD_ADMIN_ROLE_PERMISSIONS, " ORDER BY id LIMIT 100"];
        pub SERVER_ADMIN_DATA_ROLES_SQL = [WORD_SELECT, " ARRAY[id::text, name, is_system::text, created_at::text, updated_at::text] ", WORD_FROM, " ", WORD_ADMIN_ROLES, " ORDER BY id LIMIT 100"];
        pub SERVER_ADMIN_DATA_SYSTEM_SETTINGS_SQL = [WORD_SELECT, " ARRAY[id::text, left(site_name, 8192), left(tab_title, 8192), left(main_logo, 8192), left(primary_color, 8192), left(default_admin_route, 8192), left(organization_name, 8192), left(organization_contacts, 8192), left(support_url, 8192), updated_at::text] ", WORD_FROM, " ", WORD_ADMIN_SYSTEM_SETTINGS, " ORDER BY id LIMIT 100"];
        pub SERVER_ADMIN_DATA_USER_MFA_SQL = [WORD_SELECT, " ARRAY[user_id::text, pending_created_at::text, enabled_at::text, updated_at::text, last_totp_counter::text] ", WORD_FROM, " ", WORD_ADMIN_USER_MFA, " ORDER BY user_id LIMIT 100"];
        pub SERVER_ADMIN_DATA_USER_ROLES_SQL = [WORD_SELECT, " ARRAY[id::text, user_id::text, role_id::text, created_at::text] ", WORD_FROM, " ", WORD_ADMIN_USER_ROLES, " ORDER BY id LIMIT 100"];
        pub SERVER_ADMIN_DATA_USERS_SQL = [WORD_SELECT, " ARRAY[id::text, login, display_name, is_banned::text, created_at::text, updated_at::text] ", WORD_FROM, " ", WORD_ADMIN_USERS, " ORDER BY id LIMIT 100"];
