...
fn assert_commit_link_and_fallback_calls(
    ...
    let link = super::GitCommitLinkProvider::build_git_commit_link(v);
...
fn expected_git_commit_link(commit_id_src: impl AsRef<str>) -> String {
    ...
    let mut output = String::with_capacity(super::git_commit_link_capacity_value(commit_id).0);
...
fn owned_git_values_and_generated_links_enforce_length_limit() {
    ...
    let link = super::build_git_commit_link_cow(oversized.as_str());
    assert!(link.as_ref().len() <= super::GIT_INFO_STRING_MAX_LEN);
}
#[test]
fn git_commit_link_builds_expected_url() {
    let link = super::build_git_commit_link(constants_str::TEST_VALUES_COMMIT);
    assert_expected_git_commit_link(&link, constants_str::TEST_VALUES_COMMIT);
}
#[test]
fn git_commit_link_supports_empty_commit() {
    let link = super::build_git_commit_link(constants_str::PG_CRUD_EMPTY_SQL_SUFFIX);
...
fn git_commit_link_cow_borrows_static_project_link_for_project_commit() {
    let project_commit = super::project_git_info_value().commit;
    let actual = super::build_git_commit_link_cow(project_commit);
    assert!(
        matches!(actual.0, std::borrow::Cow::Borrowed(v) if std::ptr::eq(v, super::project_git_commit_link_ref_value().0))
    );
}
#[test]
fn git_commit_link_uses_static_project_link_for_project_commit() {
    let project_commit = super::project_git_info_value().commit;
    let actual = super::build_git_commit_link(project_commit);
    assert_eq!(actual, super::project_git_commit_link_ref_value());
}
#[test]
fn git_commit_link_cow_owns_link_for_non_project_commit() {
    let actual = super::build_git_commit_link_cow(constants_str::TEST_VALUES_WRONG_COMMIT);
    assert!(
...
fn is_project_commit_returns_true_for_project_commit() {
    assert!(super::check_is_project_commit(
        super::project_git_info_value().commit
    ));
}
#[test]
fn is_project_commit_returns_false_for_other_commit() {
    assert!(!super::check_is_project_commit("deadbeef"));
}
#[test]
fn validate_project_commit_returns_ok_for_project_commit() {
    assert_eq!(
        super::validate_project_commit(super::project_git_info_value().commit),
        Ok(())
    );
}
#[test]
fn validate_project_commit_returns_project_link_for_non_project_commit() {
    assert_eq!(
        ...
        Err(super::ValidateProjectCommitError(
            super::project_git_commit_link_ref_value()
        ))
    );
}
#[test]
fn validate_project_commit_reuses_static_project_link_ref() {
    ...
    let project_link = super::project_git_commit_link_ref_value();
...
fn project_git_commit_link_matches_project_commit() {
    assert_eq!(
        ...
        expected_git_commit_link(super::project_git_info_value().commit)
    );
}
#[test]
fn project_git_commit_link_ref_is_static_and_stable() {
    let first = super::project_git_commit_link_ref_value();
    let second = super::project_git_commit_link_ref_value();
...
fn project_git_info_returns_commit_link() {
    ...
    let link = super::GitCommitLinkProvider::build_git_commit_link(&git_info);
...
fn git_commit_link_calls_allocating_fallback_once_without_ref() {
    let test_git_commit = make_owned_test_git_commit(constants_str::F00DBABE);
    drop(super::GitCommitLinkProvider::build_git_commit_link(
        &test_git_commit,
    ));
...
fn git_commit_link_cow_borrows_project_link_for_project_commit() {
    let git_info = super::ProjectGitInfo {
        commit: super::project_git_info_value().commit,
    };
    let link = super::GitCommitLinkProvider::build_git_commit_link_cow(&git_info);
    assert!(
        matches!(link.0, std::borrow::Cow::Borrowed(v) if std::ptr::eq(v, super::project_git_commit_link_ref_value().0))
    );
}
#[test]
...
fn base_git_commit_link_len_matches_expected_prefix_len() {
    ...
    assert_eq!(super::git_commit_link_capacity_value(commit_id), expected);
}
#[test]
fn git_commit_link_works_for_str_and_string() {
    let str_link =
        super::GitCommitLinkProvider::build_git_commit_link(constants_str::TEST_VALUES_COMMIT);
    ...
    let string_link = super::GitCommitLinkProvider::build_git_commit_link(&string);
...
fn git_commit_link_works_for_cow_str() {
    ...
    let borrowed_link = super::GitCommitLinkProvider::build_git_commit_link(&borrowed);
    ...
    assert_expected_git_commit_link(
        super::GitCommitLinkProvider::build_git_commit_link(&owned),
...
fn git_commit_link_capacity_supports_empty_commit() {
    assert_eq!(
        super::git_commit_link_capacity_value(""),
