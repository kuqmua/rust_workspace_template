...
impl crate::domain_types::GitCommitIdProvider for TestGitCommit {
    fn git_commit_id(&self) -> crate::domain_types::GitCommitId {
        let calls = self.fallback_calls.get().saturating_add(1);
        self.fallback_calls.set(calls);
        crate::domain_types::GitCommitId::try_from(self.commit.to_owned())
            .expect("45a9c31d git_commit_id invariant must hold")
    }
    fn git_commit_id_ref(&self) -> Option<crate::domain_types::GitCommitIdRef<'_>> {
        self.borrow_commit_ref
            .then_some(crate::domain_types::GitCommitIdRef::from(self.commit))
    }
}
...
fn assert_commit_link_and_fallback_calls(
    ...
    let link = crate::domain_types::GitCommitLinkProvider::build_git_commit_link(v);
...
fn assert_commit_id_cow_and_fallback_calls(
    ...
    let commit_id = crate::domain_types::GitCommitIdProvider::git_commit_id_cow(v);
...
fn assert_commit_len_and_fallback_calls(
    ...
    let commit_len = crate::domain_types::GitCommitIdProvider::with_git_commit_id(v, |commit_id| {
        commit_id.0.len()
    });
...
fn assert_with_git_commit_id_ref_or(
    ...
    let commit_len = crate::domain_types::with_git_commit_id_ref_or(
        v,
        |commit_id| commit_id.0.len(),
        |src| {
            crate::domain_types::GitCommitIdProvider::git_commit_id(src)
                .0
                .len()
        },
    );
...
fn expected_git_commit_link(commit_id_src: impl AsRef<str>) -> String {
    let commit_id = crate::domain_types::GitCommitIdRef::from(commit_id_src.as_ref());
    let mut output =
        String::with_capacity(crate::domain_types::git_commit_link_capacity_value(commit_id).0);
    let mut output_ref = crate::domain_types::GitCommitLinkOutputRefMut::from(&mut output);
    crate::domain_types::write_git_commit_link(&mut output_ref, commit_id);
    output
}
#[test]
fn owned_git_values_and_generated_links_enforce_length_limit() {
    let oversized = constants_str::X
        .repeat(crate::domain_types::GIT_INFO_STRING_MAX_LEN + constants_usize::ONE);
    let Err(_commit_id_error) =
        crate::domain_types::GitCommitIdCow::try_from(std::borrow::Cow::Owned(oversized.clone()))
    else {
        panic!("8c811508");
    };
    let Err(_commit_link_error) =
        crate::domain_types::GitCommitLinkCow::try_from(std::borrow::Cow::Owned(oversized.clone()))
    else {
        panic!("69ee1326");
    };
    let commit = crate::domain_types::GitCommitIdProvider::git_commit_id(oversized.as_str());
    assert!(commit.as_ref().len() <= crate::domain_types::GIT_INFO_STRING_MAX_LEN);
    let link = crate::domain_types::build_git_commit_link_cow(oversized.as_str());
    assert!(link.as_ref().len() <= crate::domain_types::GIT_INFO_STRING_MAX_LEN);
}
#[test]
fn git_commit_link_builds_expected_url() {
    let link = crate::domain_types::build_git_commit_link(constants_str::TEST_VALUES_COMMIT);
    assert_expected_git_commit_link(&link, constants_str::TEST_VALUES_COMMIT);
}
#[test]
fn git_commit_link_supports_empty_commit() {
    let link = crate::domain_types::build_git_commit_link(constants_str::PG_CRUD_EMPTY_SQL_SUFFIX);
...
fn git_commit_link_cow_borrows_static_project_link_for_project_commit() {
    let project_commit = crate::domain_types::project_git_info_value().commit;
    let actual = crate::domain_types::build_git_commit_link_cow(project_commit);
    assert!(
        matches!(actual.0, std::borrow::Cow::Borrowed(v) if std::ptr::eq(v, crate::domain_types::project_git_commit_link_ref_value().0))
    );
}
#[test]
fn git_commit_link_uses_static_project_link_for_project_commit() {
    let project_commit = crate::domain_types::project_git_info_value().commit;
    let actual = crate::domain_types::build_git_commit_link(project_commit);
    assert_eq!(
        actual,
        crate::domain_types::project_git_commit_link_ref_value()
    );
}
#[test]
fn git_commit_link_cow_owns_link_for_non_project_commit() {
    let actual =
        crate::domain_types::build_git_commit_link_cow(constants_str::TEST_VALUES_WRONG_COMMIT);
    assert!(
...
fn is_project_commit_returns_true_for_project_commit() {
    assert!(crate::domain_types::check_is_project_commit(
        crate::domain_types::project_git_info_value().commit
    ));
}
#[test]
fn is_project_commit_returns_false_for_other_commit() {
    assert!(!crate::domain_types::check_is_project_commit("deadbeef"));
}
#[test]
fn validate_project_commit_returns_ok_for_project_commit() {
    assert_eq!(
        crate::domain_types::validate_project_commit(
            crate::domain_types::project_git_info_value().commit
        ),
        Ok(())
    );
...
fn validate_project_commit_returns_project_link_for_non_project_commit() {
    assert_eq!(
        crate::domain_types::validate_project_commit("deadbeef"),
        Err(crate::domain_types::ValidateProjectCommitError(
            crate::domain_types::project_git_commit_link_ref_value()
        ))
    );
}
...
fn validate_project_commit_reuses_static_project_link_ref() {
    let error =
        crate::domain_types::validate_project_commit(constants_str::TEST_VALUES_WRONG_COMMIT)
            .expect_err(constants_str::VALUE_46BC13A9);
    let project_link = crate::domain_types::project_git_commit_link_ref_value();
    assert!(std::ptr::eq(error.0.0, project_link.0));
}
#[test]
fn project_git_commit_link_matches_project_commit() {
    assert_eq!(
        crate::domain_types::project_git_commit_link(),
        expected_git_commit_link(crate::domain_types::project_git_info_value().commit)
    );
}
#[test]
fn project_git_commit_link_ref_is_static_and_stable() {
    let first = crate::domain_types::project_git_commit_link_ref_value();
    let second = crate::domain_types::project_git_commit_link_ref_value();
    assert_eq!(first, second);
    assert!(std::ptr::eq(first.0, second.0));
}
#[test]
fn project_git_info_returns_commit_link() {
    let git_info = crate::domain_types::ProjectGitInfo {
        commit: crate::domain_types::GitCommitIdRef::from(constants_str::TEST_VALUES_WRONG_COMMIT),
    };
    let link = crate::domain_types::GitCommitLinkProvider::build_git_commit_link(&git_info);
...
fn git_commit_link_calls_allocating_fallback_once_without_ref() {
    let test_git_commit = make_owned_test_git_commit(constants_str::F00DBABE);
    drop(crate::domain_types::GitCommitLinkProvider::build_git_commit_link(&test_git_commit));
    assert_fallback_calls(&test_git_commit, 1);
}
#[test]
fn git_commit_id_or_else_computes_fallback_once() {
    let test_git_commit = make_owned_test_git_commit(constants_str::F00DBABE);
    let mut fallback = crate::domain_types::GitCommitIdFallback::from(None);
    let first = crate::domain_types::GitCommitIdProvider::git_commit_id_or_else(
        &test_git_commit,
        &mut fallback,
    );
    assert_eq!(first, "f00dbabe");
    let second = crate::domain_types::GitCommitIdProvider::git_commit_id_or_else(
        &test_git_commit,
        &mut fallback,
    );
...
fn git_commit_id_or_else_prefers_borrowed_ref_without_fallback() {
    ...
    let mut fallback = crate::domain_types::GitCommitIdFallback::from(None);
    let commit = crate::domain_types::GitCommitIdProvider::git_commit_id_or_else(
        &test_git_commit,
        &mut fallback,
    );
...
fn git_commit_link_cow_borrows_project_link_for_project_commit() {
    let git_info = crate::domain_types::ProjectGitInfo {
        commit: crate::domain_types::project_git_info_value().commit,
    };
    let link = crate::domain_types::GitCommitLinkProvider::build_git_commit_link_cow(&git_info);
    assert!(
        matches!(link.0, std::borrow::Cow::Borrowed(v) if std::ptr::eq(v, crate::domain_types::project_git_commit_link_ref_value().0))
    );
}
#[test]
...
fn base_git_commit_link_len_matches_expected_prefix_len() {
    ...
    assert_eq!(
        crate::domain_types::git_commit_link_capacity_value(commit_id),
        expected
    );
}
#[test]
fn git_commit_link_works_for_str_and_string() {
    let str_link = crate::domain_types::GitCommitLinkProvider::build_git_commit_link(
    ...
    let string_link = crate::domain_types::GitCommitLinkProvider::build_git_commit_link(&string);
...
fn git_commit_link_works_for_cow_str() {
    ...
    let borrowed_link =
        crate::domain_types::GitCommitLinkProvider::build_git_commit_link(&borrowed);
    ...
    assert_expected_git_commit_link(
        crate::domain_types::GitCommitLinkProvider::build_git_commit_link(&owned),
        constants_str::TEST_VALUES_COMMIT,
    );
}
#[test]
fn project_git_info_as_ref_returns_commit() {
    let info = crate::domain_types::ProjectGitInfo {
        commit: crate::domain_types::GitCommitIdRef::from(constants_str::TEST_VALUES_COMMIT),
    };
...
fn git_commit_link_capacity_supports_empty_commit() {
    assert_eq!(
        crate::domain_types::git_commit_link_capacity_value(""),
