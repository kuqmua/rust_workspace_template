...
async fn remote_source_implements_synchronization_source_contract() {
    let mut source = crate::RemoteSyncSource::new(
        synchronization_service_runtime::SynchronizationPayload::try_from(vec![1u8, 2u8]).expect(
...
async fn notification_provider_records_messages_through_runtime_contract() {
    let (provider, mut inbox) = crate::create_mock_notification_provider();
