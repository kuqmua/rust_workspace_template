pub fn build_generate_pg_types(
    parsed_generate_pg_types_config: crate::parsed_generate_pg_types_config::ParsedGeneratePgTypesConfig,
) -> Result<
...
> {
    let entry_count = crate::pg_types_model_entry_count::PgTypesModelEntryCount::from(
        match parsed_generate_pg_types_config.get_inner().get_variant() {
    ...
    Ok(
        crate::built_generate_pg_types_model::BuiltGeneratePgTypesModel::new(
            crate::generate_pg_types_config::GeneratePgTypesConfig::from(
                parsed_generate_pg_types_config,
            ),
            entry_count,
        ),
    )
