...
fn access_token_round_trip_checks_issuer_and_audience() {
    let claims = super::AdminAccessClaims::new(
        ...
        config_lib::AdminTokenIssuer::try_from(constants_str::TEST_ISSUER.to_owned()).expect(
            "fd6a65b0 access_token_round_trip_checks_issuer_and_audience invariant must hold",
        ),
        config_lib::AdminTokenAudience::try_from(constants_str::TEST_AUDIENCE.to_owned()).expect(
            "6e423e16 access_token_round_trip_checks_issuer_and_audience invariant must hold",
        ),
    );
