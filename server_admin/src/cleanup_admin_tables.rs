pub async fn cleanup_admin_tables(
    ...
    admin_cleanup_configuration: crate::admin_cleanup_configuration::AdminCleanupConfiguration,
) -> Result<
...
> {
    let access_sessions = sqlx::query(constants_str::SERVER_ADMIN_CLEANUP_ACCESS_SESSIONS_SQL)
        .bind(admin_cleanup_configuration.auth_retention().get())
    ...
    let login_attempts = sqlx::query(constants_str::SERVER_ADMIN_CLEANUP_LOGIN_ATTEMPTS_SQL)
        ...
        .map_err(crate::admin_cleanup_error::AdminCleanupError::Pg)?
        .rows_affected();
    let mut audit_tx = sqlx::Acquire::begin(sqlx_pg_pool_ref.as_ref())
        .await
        .map_err(crate::sqlx_admin_error::SqlxAdminError::from)
        .map_err(crate::admin_cleanup_error::AdminCleanupError::Pg)?;
    ...
    let rate_limits = sqlx::query(constants_str::SERVER_ADMIN_CLEANUP_RATE_LIMITS_SQL)
        .bind(admin_cleanup_configuration.rate_limit_retention().get())
        .bind(admin_cleanup_configuration.batch_size().get())
        .execute(sqlx_pg_pool_ref.as_ref())
        .await
        .map_err(crate::sqlx_admin_error::SqlxAdminError::from)
    ...
    let idempotency = pg_table::cleanup_pg_table_idempotency::cleanup_pg_table_idempotency(
        ...
        pg_table::pg_table_idempotency_cleanup_retention_seconds::PgTableIdempotencyCleanupRetentionSeconds::try_from(
            admin_cleanup_configuration.idempotency_completed_retention().get(),
        )?,
        pg_table::pg_table_idempotency_cleanup_retention_seconds::PgTableIdempotencyCleanupRetentionSeconds::try_from(
            admin_cleanup_configuration.idempotency_pending_retention().get(),
        )?,
        pg_table::pg_table_idempotency_cleanup_batch_size::PgTableIdempotencyCleanupBatchSize::try_from(admin_cleanup_configuration.batch_size().get())?,
    )
    .await
