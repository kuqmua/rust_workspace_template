...
> {
    let access_sessions =
        sqlx::query(constants_str::integration_fixtures::SERVER_ADMIN_CLEANUP_ACCESS_SESSIONS_SQL)
            ...
            .rows_affected();
    let refresh_tokens =
        sqlx::query(constants_str::integration_fixtures::SERVER_ADMIN_CLEANUP_REFRESH_TOKENS_SQL)
            .bind(cfg.auth_retention().get())
            .bind(cfg.batch_size().get())
            .execute(pool.as_ref())
            .await
            ...
            .rows_affected();
    let login_attempts =
        sqlx::query(constants_str::integration_fixtures::SERVER_ADMIN_CLEANUP_LOGIN_ATTEMPTS_SQL)
            .bind(cfg.auth_retention().get())
            .bind(cfg.batch_size().get())
            .execute(pool.as_ref())
            .await
    ...
    let _audit_cleanup_permission =
        sqlx::query(constants_str::integration_fixtures::SERVER_ADMIN_ENABLE_AUDIT_CLEANUP_SQL)
            .execute(&mut *audit_tx)
            .await
            .map_err(crate::sqlx_admin_error::SqlxAdminError::from)
            .map_err(crate::admin_cleanup_error::AdminCleanupError::Pg)?;
    let audit_log =
        sqlx::query(constants_str::integration_fixtures::SERVER_ADMIN_CLEANUP_AUDIT_LOG_SQL)
    ...
    audit_tx
        ...
        .map_err(crate::admin_cleanup_error::AdminCleanupError::Pg)?;
    let rate_limits =
        sqlx::query(constants_str::integration_fixtures::SERVER_ADMIN_CLEANUP_RATE_LIMITS_SQL)
    ...
    sqlx::query(constants_str::integration_fixtures::SERVER_ADMIN_RECORD_CLEANUP_STATUS_SQL)
        .bind(stored_rows)
        .execute(pool.as_ref())
        .await
        .map_err(crate::sqlx_admin_error::SqlxAdminError::from)
