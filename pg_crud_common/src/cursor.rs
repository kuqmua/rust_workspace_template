...
pub struct CursorSigningKey(
    bounded_types::domain_types::vector::BoundedVec<
        u8,
        { constants_usize::ONE },
        CURSOR_SIGNING_KEY_MAXIMUM_LENGTH,
    >,
);
...
impl CursorCodec {
    ...
    pub fn encode(&self, payload: &CursorPayload) -> Result<SignedCursor, CursorEncodeError> {
        ...
        SignedCursor::try_from(cursor_text)
            .map_err(|_error| CursorEncodeError::MaximumLengthExceeded)
    }
...
mod tests {
    proptest::proptest! {
        ...
        #[cfg_attr(
            miri,
            ignore = constants_str::VALUE_BF7C931C
        )]
