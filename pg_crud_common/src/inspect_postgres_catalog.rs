...
> {
    let rows = sqlx::query(constants_str::test_fixtures::DB_SCHEMA_CATALOG_QUERY)
        .bind(schema.0)
        .fetch_all(pool.0)
        .await
        .map_err(|error| {
    ...
    rows.into_iter()
        .map(|row| {
            let kind_text: String =
                sqlx::Row::try_get(&row, constants_str::test_fixtures::OBJECT_KIND).map_err(
                    |error| {
                        crate::db_schema_conformance_error::DbSchemaConformanceError::Inspection(
            ...
            let kind = match kind_text.as_str() {
                constants_str::test_fixtures::EXTENSION => {
                    crate::db_object_kind::DbObjectKind::Extension
                }
                constants_str::test_fixtures::FUNCTION => {
                    crate::db_object_kind::DbObjectKind::Function
                }
                constants_str::test_fixtures::TRIGGER => {
                    crate::db_object_kind::DbObjectKind::Trigger
                }
                constants_str::test_fixtures::VIEW => crate::db_object_kind::DbObjectKind::View,
                _ => return Err(
                    crate::db_schema_conformance_error::DbSchemaConformanceError::UnknownObjectKind,
                ),
            };
            let name =
                sqlx::Row::try_get::<String, _>(&row, constants_str::test_fixtures::OBJECT_NAME)
                    .map_err(|error| {
                        crate::db_schema_conformance_error::DbSchemaConformanceError::Inspection(
                        crate::sqlx_db_schema_inspection_error::SqlxDbSchemaInspectionError::from(
                            error,
            ...
            let definition = sqlx::Row::try_get::<String, _>(
                &row,
                constants_str::test_fixtures::OBJECT_DEFINITION,
            )
            .map_err(|error| {
                crate::db_schema_conformance_error::DbSchemaConformanceError::Inspection(
                    crate::sqlx_db_schema_inspection_error::SqlxDbSchemaInspectionError::from(
