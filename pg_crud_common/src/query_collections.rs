...
impl<T: PartialEq> NotEmptyUniqueVec<T> {
    pub fn try_new(
        ...
        try_new_unique_vec(values, crate::take_fst_dup)
    }
}
...
impl<T: Eq + std::hash::Hash> NotEmptyUniqueVec<T> {
    pub fn try_new_by_hash(
        ...
        try_new_unique_vec(values, crate::take_fst_dup_by_hash)
    }
}
#[allow(
...
mod tests_not_empty_unique_vec {
    ...
    fn first_duplicate_idx_returns_none_for_unique_input() {
        ...
        assert!(crate::first_duplicate_idx(&values).is_none());
    }
    #[test]
    fn first_duplicate_idx_returns_none_for_empty_and_single_input() {
        assert!(crate::first_duplicate_idx::<u8>(&[]).is_none());
        assert!(crate::first_duplicate_idx(&[1u8]).is_none());
    }
    #[test]
    fn first_duplicate_idx_returns_first_repeated_value_index() {
        let values = vec![7u8, 8u8, 8u8, 7u8];
        assert_eq!(
            crate::first_duplicate_idx(&values),
            Some(crate::DuplicateIdx::from(2))
        );
    }
    #[test]
    fn first_duplicate_idx_by_hash_returns_first_repeated_value_index() {
        let values = vec![7u8, 8u8, 8u8, 7u8];
        assert_eq!(
            crate::first_duplicate_idx_by_hash(&values),
            Some(crate::DuplicateIdx::from(2))
        );
    }
    #[test]
    fn first_duplicate_idx_by_hash_returns_none_for_empty_and_single_input() {
        assert!(crate::first_duplicate_idx_by_hash::<u8>(&[]).is_none());
        assert!(crate::first_duplicate_idx_by_hash(&[1u8]).is_none());
    }
    #[test]
    fn take_fst_dup_returns_none_for_unique_input() {
        let mut values = super::super::DuplicateCandidates::from(vec![1u8, 2u8, 3u8]);
        let actual = crate::take_fst_dup(&mut values);
    ...
    fn take_fst_dup_by_hash_returns_first_duplicate_value() {
        let mut values = super::super::DuplicateCandidates::from(vec![7u8, 8u8, 8u8, 7u8]);
        let actual = crate::take_fst_dup_by_hash(&mut values);
        assert_eq!(actual, Some(8u8));
        assert_eq!(Vec::from(values).len(), 3usize);
    }
    #[test]
...
fn try_new_unique_vec<T, FindDuplicate>(
    values: super::DuplicateCandidates<T>,
    find_duplicate: FindDuplicate,
) -> Result<NotEmptyUniqueVec<T>, NotEmptyUniqueVecTryNewError<T>>
where
    FindDuplicate: FnOnce(&mut super::DuplicateCandidates<T>) -> Option<T>,
{
    ...
    if raw_values.len() > super::NOT_EMPTY_UNIQUE_VEC_MAX_LEN {
        return Err(NotEmptyUniqueVecTryNewError::TooLong {
            location: location_macros::location!(),
        });
    }
    let mut candidates = super::DuplicateCandidates::from(raw_values);
    ...
    Ok(NotEmptyUniqueVec::from(Vec::from(candidates)))
}
