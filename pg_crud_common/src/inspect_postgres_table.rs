...
> {
    let column_rows = sqlx::query(constants_str::test_fixtures::DB_SCHEMA_COLUMN_QUERY)
        .bind(schema.0)
        .bind(table.0)
        .fetch_all(pool.0)
        .await
    ...
    let columns = column_rows
        ...
        .map(|row| {
            ...
            let default: Option<String> = sqlx::Row::try_get(&row, constants_str::test_fixtures::COLUMN_DEFAULT)
                .map_err(|error| {
                crate::db_schema_conformance_error::DbSchemaConformanceError::Inspection(crate::sqlx_db_schema_inspection_error::SqlxDbSchemaInspectionError::from(error))
            })?;
            Ok(crate::db_column_snapshot::DbColumnSnapshot::new(
                ...
                .map_err(crate::db_schema_conformance_error::DbSchemaConformanceError::SchemaTextTooLong)?,
                (nullable == constants_str::test_fixtures::YES).into(),
                default
    ...
    let constraint_rows = sqlx::query(constants_str::test_fixtures::DB_SCHEMA_CONSTRAINT_QUERY)
        .bind(schema.0)
        .bind(table.0)
        .fetch_all(pool.0)
        .await
    ...
    let mut objects = constraint_rows
        .into_iter()
        .map(|row| {
            let constraint_type: String = sqlx::Row::try_get(&row, constants_str::test_fixtures::CONSTRAINT_TYPE)
                .map_err(|error| {
                    crate::db_schema_conformance_error::DbSchemaConformanceError::Inspection(crate::sqlx_db_schema_inspection_error::SqlxDbSchemaInspectionError::from(error))
                })?;
            let kind = match constraint_type.as_str() {
                constants_str::test_fixtures::DB_CONSTRAINT_CHECK => crate::db_object_kind::DbObjectKind::Check,
                constants_str::test_fixtures::DB_CONSTRAINT_FOREIGN_KEY => crate::db_object_kind::DbObjectKind::ForeignKey,
                constants_str::test_fixtures::DB_CONSTRAINT_PRIMARY_KEY => crate::db_object_kind::DbObjectKind::PrimaryKey,
                constants_str::test_fixtures::DB_CONSTRAINT_UNIQUE => crate::db_object_kind::DbObjectKind::Unique,
                _ => return Ok(None),
            };
            Ok(Some(
                crate::db_schema_text::DbSchemaText::try_from(
                    sqlx::Row::try_get::<String, _>(&row, constants_str::test_fixtures::CONSTRAINT_NAME).map_err(
                        |error| {
                            crate::db_schema_conformance_error::DbSchemaConformanceError::Inspection(crate::sqlx_db_schema_inspection_error::SqlxDbSchemaInspectionError::from(
                                error,
                            ))
                ...
                .and_then(|name| {
                    crate::db_schema_text::DbSchemaText::try_from(
                        sqlx::Row::try_get::<String, _>(&row, constants_str::test_fixtures::CONSTRAINT_DEFINITION)
                            .map_err(|error| {
                                crate::db_schema_conformance_error::DbSchemaConformanceError::Inspection(
                                    crate::sqlx_db_schema_inspection_error::SqlxDbSchemaInspectionError::from(error),
                                )
    ...
    let index_rows = sqlx::query(constants_str::test_fixtures::DB_SCHEMA_INDEX_QUERY)
        .bind(schema.0)
        .bind(table.0)
        .fetch_all(pool.0)
        .await
        ...
        index_rows
            ...
            .map(|row| {
                Ok(crate::db_object_snapshot::DbObjectSnapshot::new(
                    crate::db_schema_text::DbSchemaText::try_from(
                        sqlx::Row::try_get::<String, _>(&row, constants_str::test_fixtures::INDEX_NAME).map_err(
                            |error| {
                                crate::db_schema_conformance_error::DbSchemaConformanceError::Inspection(
                                    crate::sqlx_db_schema_inspection_error::SqlxDbSchemaInspectionError::from(error),
                                )
                    ...
                    crate::db_schema_text::DbSchemaText::try_from(
                        sqlx::Row::try_get::<String, _>(&row, constants_str::test_fixtures::INDEX_DEFINITION)
                            .map_err(|error| {
                                crate::db_schema_conformance_error::DbSchemaConformanceError::Inspection(
                                    crate::sqlx_db_schema_inspection_error::SqlxDbSchemaInspectionError::from(error),
                                )
