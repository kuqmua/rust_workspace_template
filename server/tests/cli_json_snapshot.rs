...
mod integration_tests {
    ...
    #[test]
    fn json_startup_snapshot_contract_is_stable() {
        let output =
            run_server_command_with_report_format(SERVER_BINARY_PATH, &[], "json".as_ref())
                .expect("8e1c7a2d");
        let standard_output = stdout_as_utf8(&output).expect("3d7a1e9c");

        assert!(output.status.success());
        assert_eq!(standard_output.trim_end(), "{\"status\":\"started\"}");
    }
}
