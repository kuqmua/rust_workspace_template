pub(crate) async fn make_postgresql_pool(
    ...
    if **server_config.get_pg_pool_min_connections()
        > *config_lib::pg_pool_max_connections::PgPoolMaxConnectionsProvider::pg_pool_max_connections(server_config)
    {
    ...
    sqlx::postgres::PgPoolOptions::new()
        ...
        .min_connections(**server_config.get_pg_pool_min_connections())
        .acquire_timeout(std::time::Duration::from_secs(
            server_config.get_pg_pool_acquire_timeout_seconds().get(),
        ))
        .idle_timeout(std::time::Duration::from_secs(
            server_config.get_pg_pool_idle_timeout_seconds().get(),
        ))
        .max_lifetime(std::time::Duration::from_secs(
            server_config.get_pg_pool_max_lifetime_seconds().get(),
        ))
        ...
        .connect(secrecy::ExposeSecret::expose_secret(
            config_lib::domain_types::DatabaseUrlProvider::database_url(server_config),
        ))
        .await
