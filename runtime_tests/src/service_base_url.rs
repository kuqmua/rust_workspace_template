...
impl TryFrom<String> for ServiceBaseUrl {
    ...
    fn try_from(mut value: String) -> Result<Self, Self::Error> {
        if value.len() > constants_usize::VALUE_8_192 {
            return Err(super::ServiceBaseUrlError::Length);
        }
        ...
        let parsed = match reqwest::Url::parse(value.as_str()) {
            ...
            {
                return Err(super::ServiceBaseUrlError::Host);
            }
            Err(_error) => return Err(super::ServiceBaseUrlError::Scheme),
        };
        if parsed.scheme() != constants_str::HTTP && parsed.scheme() != constants_str::HTTPS {
            return Err(super::ServiceBaseUrlError::Scheme);
        }
        if parsed.host().is_none() {
            return Err(super::ServiceBaseUrlError::Host);
        }
        if parsed.query().is_some() || parsed.fragment().is_some() {
            return Err(super::ServiceBaseUrlError::Suffix);
        }
        Ok(Self(value))
    }
}
