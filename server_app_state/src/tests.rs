...
fn make_structure(project_git_info: git_info::ProjectGitInfo<'_>) -> crate::ServerAppState<'_> {
    crate::ServerAppState {
        ...
        config: server_config::config::Config {
            ...
            pg_pool_max_connections: config_lib::domain_types::PgPoolMaxConnections::try_from(7)
        ...
        idempotency_response_budget: server_runtime_core::ResourceBudget::new(
            server_runtime_core::ResourceBudgetMaximum::try_from(constants_usize::VALUE_1_048_576)
...
async fn sqlx_pg_pool_returns_same_pool_ref() {
    let git_info = make_git_info();
    let structure = make_structure(git_info);
    let lhs = std::ptr::from_ref(app_state::SqlxPgPoolProvider::sqlx_pg_pool(&structure).as_ref());
