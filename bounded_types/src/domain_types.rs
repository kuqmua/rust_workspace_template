...
mod tests {
    ...
    fn string_bounds_are_inclusive() {
        let value =
            super::text::BoundedString::<1, 3>::try_from(constants_str::ABC_ALT_3.to_owned())
    ...
    fn unbounded_byte_string_schema_omits_max_bytes_extension() {
        let schema =
            <super::text::BoundedString<1, { usize::MAX }> as utoipa::PartialSchema>::schema();
        let utoipa::openapi::RefOr::T(utoipa::openapi::schema::Schema::Object(object)) = schema
        else {
            panic!("43fbea64");
        };
    ...
    fn max_vec_construction_preserves_order_and_supports_consuming_iteration() {
        let values =
            super::vector::BoundedVec::<u8, 0, { usize::MAX }>::from_max_iter([3u8, 1u8, 2u8]);
    ...
    fn map_deserialization_enforces_capacity_and_allows_duplicate_replacement() {
        ...
        let result = <super::hash::BoundedHashMap<u8, u8, 1> as serde::Deserialize>::deserialize(
            distinct_map,
        );
