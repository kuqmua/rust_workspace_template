...
pub(super) fn admin_data_grid_row(
    ...
    let cells = row.values().iter().enumerate().map(|(index, value)| {
        ...
        leptos::view! { <crate::domain_types::with_owner::table::table_cell::TableCell data_label=label data_field=field class=if numeric { "numeric-cell" } else { "" }>{value_text}</crate::domain_types::with_owner::table::table_cell::TableCell> }
    }).collect::<Vec<_>>();
    leptos::view! {
        <crate::domain_types::with_owner::table::table_row::TableRow>{cells}</crate::domain_types::with_owner::table::table_row::TableRow>
    }
}
