...
pub(super) fn admin_data_grid_row(
    ...
    let cells = row.values().iter().enumerate().map(|(index, value)| {
        ...
        leptos::view! { <crate::ui::table::TableCell data_label=label data_field=field class=if numeric { "numeric-cell" } else { "" }>{value_text}</crate::ui::table::TableCell> }
    }).collect::<Vec<_>>();
    leptos::view! {
        <crate::ui::table::TableRow>{cells}</crate::ui::table::TableRow>
    }
}
