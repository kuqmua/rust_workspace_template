...
pub(super) fn users_view(
    ...
    <TableTools state sort_options=&str_constants::admin_table::USER_SORTS />
...
pub(super) fn permissions_view(
    ...
    let rows = move || {
        ...
        filtered.sort_by(|left, right| {
            let order = if state_value.sort().0 == str_constants::sql_names::ID {
