...
fn EditUserForm(
    ...
    let pending = RwSignal::new(false);
    let error = RwSignal::<Option<String>>::new(None);
    view! { <details class="mutation-form compact"><summary>"Edit"</summary><form on:submit=move |event| {
        event.prevent_default();
...
fn UserRoleEditor(
    ...
    view! {
        <details class="assignment-editor">
            ...
            {move || error.get().map(|value| view! { <p class="field-error" role="alert">{value}</p> })}<button disabled=move || !can_update || pending.get() on:click=move |_| { let body = server_admin_contract::AdminSetUserRolesReq::from_ids(selected.get()); let action_client = client.clone(); run_mutation(action_client.clone().send_json(server_admin_contract::AdminRoute::SetUserRoles(user_id), body), pending, error, "User roles updated", action_client, loader); }>{move || if pending.get() { "Saving..." } else { "Save roles" }}</button>
        </details>
    }
}
#[component]
fn RolePermissionEditor(
    ...
    view! {
        <details class="assignment-editor">
            ...
            {move || error.get().map(|value| view! { <p class="field-error" role="alert">{value}</p> })}<button disabled=move || !can_update || pending.get() on:click=move |_| { let body = server_admin_contract::AdminSetRolePermissionsReq::from_ids(selected.get()); let action_client = client.clone(); run_mutation(action_client.clone().send_json(server_admin_contract::AdminRoute::SetRolePermissions(role_id), body), pending, error, "Role permissions updated", action_client, loader); }>{move || if pending.get() { "Saving..." } else { "Save permissions" }}</button>
        </details>
    }
}
...
fn AuditFilters(
    ...
    let created_after = RwSignal::new(super::query_value("created_after"));
    let created_before = RwSignal::new(super::query_value("created_before"));
    let user_login = RwSignal::new(super::query_value("user_login"));
    let action = RwSignal::new(super::query_value("action"));
    let resource = RwSignal::new(super::query_value("resource"));
    let resource_id = RwSignal::new(super::query_value("resource_id"));
    let succeeded = RwSignal::new(super::query_value("succeeded"));
    let limit = RwSignal::new(match super::query_value("limit").as_str() {
        "10" | "50" | "100" => super::query_value("limit"),
        _ => "20".to_owned(),
    });
...
pub(super) fn audit_view(
    ...
    let content = view! { <div class="crud-content"><TableTools state sort_options=&server_admin_contract::AdminTableSortField::AUDIT /><table><thead><tr><th>"Time"</th><th>"User"</th><th>"Action"</th><th>"Resource"</th><th>"Result"</th><th>"Details"</th></tr></thead><tbody>{move || { let all_rows = rows(); let current = state.get(); let start = current.start(crate::table_state::AdminFrontendTableIndex::from(all_rows.len())).0; let end = current.end(crate::table_state::AdminFrontendTableIndex::from(all_rows.len())).0; all_rows[start..end].iter().cloned().map(|value| {
        ...
        let details = value.details().map(ToString::to_string).unwrap_or_else(|| "No additional details".to_owned());
        view! { <tr><td>{value.created_at().to_string()}</td><td>{user}</td><td>{value.action().to_string()}</td><td>{resource}</td><td>{value.succeeded().to_string()}</td><td><details class="audit-event"><summary>{format!("Event #{event_id}")}</summary><pre>{details}</pre></details></td></tr> }
