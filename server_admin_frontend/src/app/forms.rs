        ...
        <form class="auth-form" on:submit=move |event| {
            ...
            leptos::task::spawn_local(async move {
                ...
                match client.sign_in(input).await {
                    Ok(value) => { let _display_name = value.user().display_name(); super::redirect(str_constants::ADMIN_PAGE_PATHS_ROOT); },
...
pub(super) fn settings_view(
    ...
    <label><span>"Default admin route"</span><input placeholder=str_constants::ADMIN_PAGE_PATHS_USERS prop:value=move || default_admin_route.get() on:input=move |event| default_admin_route.set(event_target_value(&event)) /></label>
