...
pub(crate) async fn fetch_role_rules_read(
...
> {
    ...
    let input_kind = match filter_query.field() {
        ...
        Some(field)
            if matches!(
                field.as_ref(),
                constants_str::SQL_NAMES_ID | constants_str::ROLE_ID | constants_str::RULE_ID
            ) =>
        {
    ...
    let request =
        server_admin_contract::admin_role_rules_read_request::AdminRoleRulesReadRequest::new(
