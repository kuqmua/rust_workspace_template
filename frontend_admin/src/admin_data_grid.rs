...
pub(crate) fn AdminDataGrid(
    ...
    leptos::view! {
        <section class="table-page" data-renderer="csr">
            {can_export.then(|| leptos::view! { <crate::admin_audit_download::AdminAuditDownload admin_page_limit=admin_csr_query.limit() admin_page_offset=admin_csr_query.offset() /> })}
            {grid}
