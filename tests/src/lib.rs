    ...
    mod code_style {
        ...
        impl<'ast> syn::visit::Visit<'ast> for RuntimeArcVisitor {
            fn visit_expr_call(&mut self, i: &'ast syn::ExprCall) {
                ...
                {
                    self.ers.push(
                        "Arc::new() outside approved cross-thread state construction".to_owned(),
                    );
                }
        ...
        fn compare_lints_vecs(
            ...
            let (lints_not_in_cargo_toml, lints_missing_by_exception) =
                split_lints_missing_from_cargo(
                    lints_to_check,
        ...
        fn env_and_envexample_have_same_keys() {
            ...
            let mut ers =
                collect_missing_key_ers(&env_keys, &example_keys_set, ".env", ".envexample");
        ...
        fn for_each_rs_file_content(mut on_file: impl FnMut(&std::path::Path, &str)) {
            rs_project_files()
                ...
                .for_each(|(entry, v)| on_file(entry.path(), &v));
        }
                ...
                ["dependencies", "dev-dependencies", "build-dependencies"]
                    ...
                    .flat_map(|(dep_section, deps)| {
                        deps.iter()
                            ...
                            .map(move |(dep_name, _)| {
                                workspace_dep_entry_er(path, dep_name, dep_section)
                            })
                    }),
            );
        }
