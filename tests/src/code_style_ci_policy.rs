...
fn continuous_integration_contains_required_security_and_quality_commands() {
    ...
    [
        constants_str::catalog::PERMISSIONS_NEWLINE_CONTENTS_READ,
        constants_str::catalog::RHYSD_ACTIONLINT,
        constants_str::catalog::CARGO_MACHETE,
        constants_str::catalog::CARGO_LLVM_COV_WORKSPACE_ALL_FEATURES_SUMMARY_ONLY,
        constants_str::catalog::AQUASECURITY_TRIVY_ACTION,
        constants_str::catalog::CARGO_PLUS_NIGHTLY_UDEPS_WORKSPACE_ALL_TARGETS_ALL_FEATURES_LOCKED,
    ]
    .into_iter()
...
fn continuous_integration_runs_specialized_test_families() {
    let workflow = workflow();
    [
        constants_str::catalog::CI_MIRI_COMPONENT,
        constants_str::catalog::CI_MIRI_TEST_CMD,
        constants_str::catalog::CI_DATABASE_TEST_CMD,
        constants_str::catalog::CI_BROWSER_TEST_CMD,
    ]
    .into_iter()
    .for_each(|required| {
        assert!(
...
fn continuous_integration_uses_the_pinned_workspace_toolchain() {
    ...
    let toolchain_source = std::fs::read_to_string(
        repository_root.join(constants_str::test_fixtures::VALUE_2B1BDE2C),
    )
    .expect(
    ...
    let toolchain = parsed_toolchain
        .get(constants_str::test_fixtures::VALUE_0DB3DE82)
        .and_then(toml::Value::as_table)
        .and_then(|table| table.get(constants_str::test_fixtures::VALUE_69E36568))
    ...
    let setup_action = std::fs::read_to_string(
        repository_root.join(constants_str::test_fixtures::VALUE_D8346474),
    )
    .expect(
...
fn workflow_jobs_have_timeouts_and_marketplace_actions_use_commit_shas() {
    ...
    let workflow_jobs = workflow
        .as_ref()
        .split_once(constants_str::catalog::JOBS_NEWLINE)
    ...
    workflow_jobs.lines().for_each(|line| {
        ...
        if inside_job
            && line.starts_with(constants_str::catalog::FOUR_SPACES)
            && !line
                .strip_prefix(constants_str::catalog::FOUR_SPACES)
                .is_some_and(|remainder| remainder.starts_with(constants_str::catalog::TWO_SPACES))
            && line
                .trim_start()
                .starts_with(constants_str::catalog::TIMEOUT_MINUTES)
        {
            current_job_has_timeout = true;
        }
        if let Some(action) = line.trim().strip_prefix(constants_str::catalog::USES) {
            if action.starts_with(constants_str::test_fixtures::VALUE_C14CECEC) {
                return;
            }
...
fn workflow_policy_ignores_commented_commands_and_actions() {
    let source = active_workflow_source(crate::types::SourceTextRef::from(
        constants_str::test_fixtures::VALUE_0356E8E3,
    ));
    assert!(
        !source
            .as_ref()
            .contains(constants_str::catalog::CARGO_MACHETE)
    );
    assert!(
        !source
            .as_ref()
            .contains(constants_str::catalog::TIMEOUT_MINUTES)
    );
