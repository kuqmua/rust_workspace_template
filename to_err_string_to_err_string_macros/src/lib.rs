...
pub fn impl_to_err_string_with(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    let Some((types_raw, closure)) = workspace_macro_helpers::domain_types::split_fat_arrow(
    ...
    let Some((value, body)) =
        workspace_macro_helpers::domain_types::closure_identifier_and_body(closure)
    else {
        return workspace_macro_helpers::domain_types::compile_error_token_stream(
    ...
    let types = workspace_macro_helpers::domain_types::split_top_level_commas(types_raw)
        .into_iter()
...
pub fn impl_to_err_string_const(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    let parsed_pairs_res = workspace_macro_helpers::domain_types::split_top_level_commas(
        workspace_macro_helpers::domain_types::ProcMacro2MacroTokens::from_into(input),
    )
    .into_iter()
    .filter(|part| !part.is_empty())
    .map(|part| {
        workspace_macro_helpers::domain_types::split_fat_arrow(part)
...
pub fn impl_to_err_string_as_ref_str(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    let types = workspace_macro_helpers::domain_types::split_top_level_commas(
        workspace_macro_helpers::domain_types::ProcMacro2MacroTokens::from_into(input),
    )
    .into_iter()
    .filter(|part| !part.is_empty())
