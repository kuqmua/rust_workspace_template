...
impl syn::parse::Parse for WireEnumAttrs {
    fn parse(input: syn::parse::ParseStream<'_>) -> syn::Result<Self> {
        ...
        while !input.is_empty() {
            ...
            if name == constants_str::test_fixtures::WIRE_ENUM_REF_TYPE {
                ref_type = Some(crate::syn_type::SynType::from(input.parse::<syn::Type>()?));
            } else if name == constants_str::test_fixtures::WIRE_ENUM_ERROR_MESSAGE {
        ...
        Ok(Self {
            error_message: error_message.ok_or_else(|| {
                input.error(constants_str::test_fixtures::WIRE_ENUM_REQUIRES_ATTRIBUTE)
            })?,
            ref_type: ref_type.ok_or_else(|| {
                input.error(constants_str::test_fixtures::WIRE_ENUM_REQUIRES_ATTRIBUTE)
            })?,
        })
    }
