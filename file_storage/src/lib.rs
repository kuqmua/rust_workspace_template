...
mod tests {
    ...
    async fn stale_staging_cleanup_is_bounded_and_removes_regular_files() {
        ...
        let storage = super::SafeFileStorage::new(
            super::StdFileStorageRoot::try_from(root_path.clone()).expect("0a4c0bfd stale_staging_cleanup_is_bounded_and_removes_regular_files invariant must hold"),
        );
    ...
    async fn staged_upload_delete_and_rollback_preserve_transaction_boundaries() {
        ...
        let storage = super::SafeFileStorage::new(
            super::StdFileStorageRoot::try_from(root_path.clone()).expect("ec6f4321 staged_upload_delete_and_rollback_preserve_transaction_boundaries invariant must hold"),
        );
