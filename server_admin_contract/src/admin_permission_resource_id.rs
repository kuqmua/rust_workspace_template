...
pub struct AdminPermissionResourceId(crate::positive_non_zero_i64::PositiveNonZeroI64);

impl TryFrom<i64> for AdminPermissionResourceId {
...
impl From<AdminPermissionResourceId> for i64 {
    fn from(value: AdminPermissionResourceId) -> Self {
        value.0.get()
    }
}
...
mod tests {
    #[test]
    fn test_permission_resource_identifier_parses_from_detail_frontend_path() {
        assert_eq!(
            super::AdminPermissionResourceId::try_from(0i64),
            Err(crate::admin_id_try_from_i64_error::AdminIdTryFromI64Error::Invalid)
        );
        let identifier_result = super::AdminPermissionResourceId::try_from(7i64);
        identifier_result.ok().into_iter().for_each(|identifier| {
            let route_path = crate::admin_route_path::AdminRoutePath::from(identifier);
            assert_eq!(
                super::AdminPermissionResourceId::from_frontend_path(
