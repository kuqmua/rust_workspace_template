...
pub struct AdminRoleRuleId(crate::positive_non_zero_i64::PositiveNonZeroI64);
impl TryFrom<i64> for AdminRoleRuleId {
...
impl From<AdminRoleRuleId> for i64 {
    fn from(value: AdminRoleRuleId) -> Self {
        value.0.get()
    }
}
impl AdminRoleRuleId {
    #[must_use]
    pub fn from_frontend_path(
        admin_page_path_ref: crate::admin_page_path_ref::AdminPagePathRef<'_>,
    ) -> Option<Self> {
        admin_page_path_ref
            .record_id(crate::admin_data_table::AdminDataTable::RoleRules)
            .map(Self::from)
    }
}
...
mod tests {
    #[test]
    fn test_role_rule_identifier_parses_from_detail_frontend_path() {
        assert_eq!(
            super::AdminRoleRuleId::try_from(0i64),
            Err(crate::admin_id_try_from_i64_error::AdminIdTryFromI64Error::Invalid)
        );
        assert_eq!(
            super::AdminRoleRuleId::try_from(-1i64),
            Err(crate::admin_id_try_from_i64_error::AdminIdTryFromI64Error::Invalid)
        );
        let expected_identifier_result = super::AdminRoleRuleId::try_from(7i64);
        assert_eq!(
