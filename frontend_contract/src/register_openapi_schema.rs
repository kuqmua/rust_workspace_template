    ...
    Schema: utoipa::ToSchema,
{
    ...
    let _previous_qualified_schema = utoipa_open_api_components_ref_mut
        .schemas
        .insert(qualified_name, schema.clone());
    if let Some(crate_name) = std::any::type_name::<Schema>()
        .split(constants_str::DOUBLE_COLON)
        .next()
    {
        let _previous_crate_schema = utoipa_open_api_components_ref_mut
            .schemas
            .insert(format!("{crate_name}.{name}"), schema.clone());
    }
    let _previous_named_schema = utoipa_open_api_components_ref_mut
        .schemas
        .insert(name.into_owned(), schema);
    let mut referenced_schemas = Vec::new();
    Schema::schemas(&mut referenced_schemas);
    referenced_schemas
        .into_iter()
        .for_each(|(referenced_name, referenced_schema)| {
            let _previous_schema = utoipa_open_api_components_ref_mut
                .schemas
