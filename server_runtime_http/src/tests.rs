...
impl HttpErrorEventFieldVisitor {
    fn record_field(&mut self, field: &tracing::field::Field) {
        let bit = match field.name() {
            constants_str::test_fixtures::VALUE_734D11A1 => 1u16 << constants_u16::ZERO,
            constants_str::test_fixtures::VALUE_E7F3C28E => 1u16 << 1u16,
            constants_str::test_fixtures::VALUE_D1BC9314 => 1u16 << 2u16,
            constants_str::test_fixtures::VALUE_660F3845 => 1u16 << 3u16,
            constants_str::test_fixtures::VALUE_990DF270 => 1u16 << 4u16,
            constants_str::test_fixtures::VALUE_0DFCA5AC => 1u16 << 5u16,
            constants_str::test_fixtures::VALUE_41A7BCE3 => 1u16 << 6u16,
            constants_str::test_fixtures::VALUE_E5BDBA7C => 1u16 << 7u16,
            constants_str::test_fixtures::VALUE_4C133E94 => 1u16 << 8u16,
            constants_str::test_fixtures::VALUE_265EE18A => 1u16 << 9u16,
            constants_str::test_fixtures::VALUE_86846B4A => 1u16 << 10u16,
            constants_str::test_fixtures::VALUE_31755A3B => 1u16 << 11u16,
...
async fn request_span_uses_remote_parent_and_server_kind() {
    ...
    let tracer = opentelemetry::trace::TracerProvider::tracer(
        &tracer_provider,
        constants_str::test_fixtures::VALUE_00BCCC04,
    );
    ...
    let trusted_proxy_ranges = crate::trusted_proxy_ranges::TrustedProxyRanges::try_from(vec![
        crate::trusted_proxy_range::TrustedProxyRange::try_from(
            constants_str::integration_fixtures::VALUE_127_0_0_1_32.to_owned(),
        )
        .expect(constants_str::test_fixtures::VALUE_3504733D),
    ])
    ...
    let router = axum::Router::from(
        crate::request_id_layer::RequestIdLayer::with_span_config(crate::http_request_span_config::HttpRequestSpanConfig::new(
            server_observability::service_name::ServiceName::from(constants_str::test_fixtures::VALUE_00BCCC04),
            crate::client_socket_addr::ClientSocketAddr::from(constants_str::catalog::VALUE_127_0_0_1_8080.parse::<std::net::SocketAddr>().expect(
        ...
        .apply(crate::axum_router::AxumRouter::from(axum::Router::new().route(
            constants_str::test_fixtures::VALUE_2C49C991,
            axum::routing::get(async || http::StatusCode::OK),
        ))),
    );
    let mut request = axum::extract::Request::builder()
        .uri(constants_str::test_fixtures::VALUE_D9F36A45)
        .header(
        ...
        .header(
            ...
            constants_str::catalog::VALUE_203_0_113_1,
        )
    ...
    let _previous_connect_info = request.extensions_mut().insert(axum::extract::ConnectInfo(
        constants_str::test_fixtures::VALUE_30010705
    ...
    let request_span = spans
        .iter()
        .find(|span| span.name == constants_str::test_fixtures::VALUE_F877E121)
    ...
    let expected_parent_span_id = constants_str::test_fixtures::TRACEPARENT_TEST_VALUE
        .get(36usize..52usize)
    ...
    assert_eq!(
        attribute(constants_str::test_fixtures::OTEL_HTTP_RESPONSE_STATUS_CODE).as_deref(),
        Some("200")
    );
    assert_eq!(
        attribute(constants_str::test_fixtures::OTEL_SERVER_ADDRESS).as_deref(),
        Some("127.0.0.1:8080")
    );
    assert_eq!(
        attribute(constants_str::test_fixtures::OTEL_CLIENT_ADDRESS).as_deref(),
        Some(constants_str::catalog::VALUE_203_0_113_1)
    );
    assert_eq!(
        attribute(constants_str::test_fixtures::OTEL_SERVICE_NAME).as_deref(),
        Some("server-runtime-test")
    );
    assert_eq!(
        attribute(constants_str::test_fixtures::OTEL_TRACE_ID).as_deref(),
        Some(expected_trace_id)
    );
    assert_eq!(
        attribute(constants_str::test_fixtures::OTEL_SPAN_ID).as_deref(),
        Some(request_span.span_context.span_id().to_string().as_str())
    );
    assert_eq!(attribute(constants_str::test_fixtures::OTEL_URL_PATH), None);
    tracer_provider
        .shutdown()
...
async fn request_span_limits_url_path_and_records_error_telemetry() {
    ...
    let tracer = opentelemetry::trace::TracerProvider::tracer(
        &tracer_provider,
        constants_str::test_fixtures::VALUE_51FC3135,
    );
    let subscriber = tracing_subscriber::layer::SubscriberExt::with(
        tracing_subscriber::registry(),
        tracing_opentelemetry::layer().with_tracer(tracer),
                ...
                let _previous = response.extensions_mut().insert(
                    crate::http_error_telemetry::HttpErrorTelemetry::new(
                        crate::http_error_type::HttpErrorType::from(
                            constants_str::test_fixtures::VALUE_E4FA4A90,
                        ),
                        crate::http_error_code::HttpErrorCode::from(
                            constants_str::test_fixtures::VALUE_69C4B56B,
                        ),
                    ),
                );
                response
    ...
    let missing_response = tower::ServiceExt::oneshot(
        router,
        axum::extract::Request::builder()
            .uri(constants_str::test_fixtures::VALUE_3BAC991E)
            .body(axum::body::Body::empty())
    ...
    let status_span = spans
        .iter()
        .find(|span| span.name == constants_str::test_fixtures::VALUE_14E536E4)
        .expect(
    ...
    assert_eq!(
        status_attribute(constants_str::test_fixtures::OTEL_URL_PATH).as_deref(),
        Some("/status")
    );
    assert_eq!(
        status_attribute(constants_str::test_fixtures::OTEL_ERROR_TYPE).as_deref(),
        Some("persistence.error")
    );
    assert_eq!(
        status_attribute(constants_str::test_fixtures::OTEL_ERROR_CODE).as_deref(),
        Some("database_unavailable")
    );
    let unmatched_span = spans
        .iter()
        .find(|span| span.name == constants_str::test_fixtures::VALUE_F607F8A1)
        .expect(
    ...
    assert!(
        unmatched_span
            .attributes
            .iter()
            .all(|attribute| attribute.key.as_str() != constants_str::test_fixtures::OTEL_URL_PATH)
    );
    ...
    assert_eq!(
        unmatched_attribute(constants_str::test_fixtures::OTEL_ERROR_TYPE).as_deref(),
        Some(constants_str::test_fixtures::OTEL_HTTP_CLIENT_ERROR_TYPE)
    );
    assert_eq!(
        unmatched_attribute(constants_str::test_fixtures::OTEL_ERROR_CODE).as_deref(),
        Some(constants_str::test_fixtures::OTEL_HTTP_4XX_ERROR_CODE)
    );
...
async fn http_boundary_emits_one_complete_error_event_only_for_server_errors() {
    ...
    let diagnostic = crate::http_error_diagnostic::HttpErrorDiagnostic::capture(
        crate::http_error_telemetry::HttpErrorTelemetry::new(
            crate::http_error_type::HttpErrorType::from(
                constants_str::test_fixtures::VALUE_D623C83D,
            ),
            crate::http_error_code::HttpErrorCode::from(
    ...
    let router = axum::Router::from(
        crate::request_id_layer::RequestIdLayer::with_span_config(crate::http_request_span_config::HttpRequestSpanConfig::new(
            server_observability::service_name::ServiceName::from(constants_str::test_fixtures::VALUE_5F735A9A),
            crate::client_socket_addr::ClientSocketAddr::from(
                constants_str::catalog::VALUE_127_0_0_1_8080
                    .parse::<std::net::SocketAddr>()
        ...
        .apply(crate::axum_router::AxumRouter::from(
            axum::Router::new()
                .route(
                    constants_str::test_fixtures::VALUE_4F613DD0,
                ...
                .route(
                    constants_str::test_fixtures::VALUE_70456F90,
    ...
    let server_error_response = tower::ServiceExt::oneshot(
        router.clone(),
        axum::extract::Request::builder()
            .uri(constants_str::test_fixtures::VALUE_4F613DD0)
            .body(axum::body::Body::empty())
    ...
    let client_error_response = tower::ServiceExt::oneshot(
        router,
        axum::extract::Request::builder()
            .uri(constants_str::test_fixtures::VALUE_70456F90)
            .body(axum::body::Body::empty())
...
fn observed_client_preparation_injects_context_and_creates_child_span() {
    ...
    let tracer = opentelemetry::trace::TracerProvider::tracer(
        &tracer_provider,
        constants_str::test_fixtures::VALUE_E0C5B098,
    );
    let subscriber = tracing_subscriber::layer::SubscriberExt::with(
        tracing_subscriber::registry(),
        tracing_opentelemetry::layer().with_tracer(tracer),
    ...
    let dispatch = tracing::Dispatch::new(subscriber);
    let _dispatch_guard = tracing::dispatcher::set_default(&dispatch);
    let url = reqwest::Url::parse(constants_str::integration_fixtures::HTTPS_EXAMPLE_COM).expect("a0c9b8a8 observed_client_preparation_injects_context_and_creates_child_span invariant must hold");
    let mut request =
        crate::reqwest_request::ReqwestRequest::from(reqwest::Request::new(http::Method::GET, url));
    let root_span = tracing::info_span!("caller");
    let prepared_client_span = root_span.in_scope(|| {
        crate::reqwest_client::ReqwestClient::prepare_observed_http_request(&mut request)
    });
    let prepared_request = request.into_inner();
    assert!(
        prepared_request
            .headers()
            .get(constants_str::test_fixtures::TRACEPARENT)
            .is_some()
    );
    ...
    let caller_span = spans
        .iter()
        .find(|span| span.name == constants_str::test_fixtures::VALUE_61DE55BA)
    ...
    let exported_client_span = spans
        .iter()
        .find(|span| span.name == constants_str::test_fixtures::VALUE_95E2AF51)
