    ...
    Service::Future: Send + 'static,
{
    ...
    fn call(&mut self, mut req: axum::extract::Request) -> Self::Future {
        let remote_context = super::extract_remote_trace_context(
            super::HttpOpentelemetryHeaderMapRef::from(req.headers()),
        );
        ...
        .find_map(|header_name| {
            req.headers().get(header_name).and_then(|value| {
                super::RequestId::try_from(value)
                    .ok()
        ...
        .unwrap_or_else(|| {
            loop {
                if let Ok(value) = super::RequestId::try_from(uuid::Uuid::new_v4().to_string())
            ...
            req.extensions()
                ...
                .map(|connect_info| {
                    super::resolve_client_ip(
                        super::HttpHeaderMapRef::from(req.headers()),
                        super::ClientSocketAddr::from(connect_info.0),
                ...
                if response.status().is_client_error() || response.status().is_server_error() {
                    let default_error_telemetry = if response.status().is_server_error() {
                        super::HttpErrorTelemetry::new(
                            super::HttpErrorType::from(constants_str::OTEL_HTTP_SERVER_ERROR_TYPE),
                            super::HttpErrorCode::from(constants_str::OTEL_HTTP_5XX_ERROR_CODE),
                        )
                    } else {
                        super::HttpErrorTelemetry::new(
                            super::HttpErrorType::from(constants_str::OTEL_HTTP_CLIENT_ERROR_TYPE),
                            super::HttpErrorCode::from(constants_str::OTEL_HTTP_4XX_ERROR_CODE),
                        )
                    };
