pub(super) fn request_origin_value_is_allowed(
    http_origin_text_ref: crate::http_origin_text_ref::HttpOriginTextRef<'_>,
    allow_origin_suffix: crate::allow_origin_suffix::AllowOriginSuffix,
    allowed_origins: &crate::allowed_origins::AllowedOrigins,
) -> crate::request_origin_allowed::RequestOriginAllowed {
    let Some(parsed) = (|| {
        let (scheme, remainder) = http_origin_text_ref
            .get()
            .trim()
        ...
        if authority.is_empty() || (!allow_origin_suffix.get() && authority_end != remainder.len())
        {
            None
        } else {
