...
impl<T, const MIN: usize, const MAX: usize> TryFrom<Vec<T>> for BoundedVec<T, MIN, MAX> {
    type Error = BoundedVecEr;
    fn try_from(value: Vec<T>) -> Result<Self, Self::Error> {
        if MIN > MAX {
            return Err(BoundedVecEr::InvalidBounds {
        ...
        if value.len() < MIN {
            Err(BoundedVecEr::BelowMin {
                actual,
                min: BoundedVecLen::from(MIN),
            })
        } else if value.len() > MAX {
            Err(BoundedVecEr::AboveMax {
                actual,
                max: BoundedVecLen::from(MAX),
            })
        } else {
    ...
    for StdPhantomDataBoundedVecVisitor<T, MIN, MAX>
{
    ...
    {
        if MIN > MAX {
            return Err(serde::de::Error::custom(BoundedVecEr::InvalidBounds {
                min: BoundedVecLen::from(MIN),
                max: BoundedVecLen::from(MAX),
            }));
        }
        ...
        while let Some(value) = seq.next_element()? {
            if values.len() == MAX {
                return Err(serde::de::Error::custom(BoundedVecEr::AboveMax {
        ...
        if values.len() < MIN {
            Err(serde::de::Error::custom(BoundedVecEr::BelowMin {
...
mod tests {
    ...
    fn try_from_enforces_inclusive_bounds() {
        assert!(matches!(
            ...
            Err(super::BoundedVecEr::BelowMin { .. })
        ));
        assert_eq!(
        ...
        assert!(matches!(
            ...
            Err(super::BoundedVecEr::AboveMax { .. })
        ));
    }
    #[test]
    fn invalid_bounds_are_rejected() {
        assert!(matches!(
            ...
            Err(super::BoundedVecEr::InvalidBounds { .. })
        ));
    }
    #[test]
