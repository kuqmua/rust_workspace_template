...
impl TryFrom<String> for SqlIdentifier {
    type Error = SqlIdentifierEr;
    fn try_from(value: String) -> Result<Self, Self::Error> {
        if value.len() > 128usize {
            return Err(SqlIdentifierEr::Invalid);
        }
        let mut bytes = value.bytes();
        let first = bytes.next().ok_or(SqlIdentifierEr::Empty)?;
...
impl TryFrom<String> for SqlQueryText {
    ...
    fn try_from(value: String) -> Result<Self, Self::Error> {
        if value.len() > crate::PG_CRUD_STRING_WRAPPER_MAX_LEN {
            Err(crate::PgCrudStringWrapperTryFromStringEr::TooLong {
                len: value.len(),
...
impl SqlSelectBuilder {
    #[must_use]
    pub fn build(self) -> crate::QpFragment {
        ...
        crate::QpFragment::try_from(query.0).unwrap_or_else(crate::QpFragment::from)
    }
    #[must_use]
