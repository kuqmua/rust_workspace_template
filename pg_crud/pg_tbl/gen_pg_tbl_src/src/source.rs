...
pub fn gen_pg_tbl(
            ...
            .try_for_each(|attr| {
                ...
                attr.parse_nested_meta(|meta| {
                    if meta.path.is_ident("filterable") {
                        frontend_flags.try_insert_with(GenPgTblFrontendFlag::Filterable, || {
                    ...
                    if meta.path.is_ident("hidden") {
                        frontend_flags.try_insert_with(GenPgTblFrontendFlag::Hidden, || {
                    ...
                    if meta.path.is_ident("sortable") {
                        frontend_flags.try_insert_with(GenPgTblFrontendFlag::Sortable, || {
    ...
    fn build_gen_pg_tbl_fields_model_stage(
        ...
        match crate::parse::struct_shape(workspace_macro_helpers::SynDeriveInputRef::from(
            ...
            Err(_error) => Err(compile_error_ts(CompileErrorMsg(
    ...
    .fold((), |(), op_dsc| {
        ...
        let open_api_method_ts = match crate::openapi::http_method(op_dsc) {
            OpHttpMethod::Post => quote::quote! {post},
            OpHttpMethod::Patch => quote::quote! {patch},
            OpHttpMethod::Delete => quote::quote! {delete},
        };
        let open_api_status_ts = if crate::openapi::success_status(op_dsc)
        ...
        if op_is_enabled(op) {
        op_routes_ts.push({
            let method_ts = match crate::route::http_method(op_dsc) {
                OpHttpMethod::Post => quote::quote! {post},
                OpHttpMethod::Patch => quote::quote! {patch},
                OpHttpMethod::Delete => quote::quote! {delete},
            };
        ...
        impl_ident_vec_ts.push({
            let try_op_ts = {
                ...
                let future_ts = {
                    let op_http_method_sc_ts =
                        naming_cmn::AsRefStrToScTs::case_or_panic(&crate::client::http_method(op_dsc));
            ...
            let op_h_ts = {
                ...
                let wraped_into_axum_res_ts = wrap_into_axum_res_ts(
                    ...
                    &crate::handler::success_status(op_dsc).to_http_status_code_ts(),
                    &AddReturn::False,
                );
    ...
    let enabled_operation_count = crate::model::OpDsc::ALL
        .iter()
        .filter(|op_dsc| op_is_enabled(&op_dsc.op))
        .count();
    let route_contract_items_ts = crate::model::OpDsc::ALL
        .iter()
        .filter(|op_dsc| op_is_enabled(&op_dsc.op))
        .map(|op_dsc| {
            let operation = quote::format_ident!("{}", op_dsc.op.to_string());
            let http_method = match crate::frontend::http_method(op_dsc) {
            ...
            let success_status = if crate::frontend::success_status(op_dsc)
                == macros_helpers::status_code::StatusCode::Crd201
            {
                ...
                .map_or_else(
                    ...
                    |permission_prefix| {
                        let permission = format!(
                            "{permission_prefix}:{}",
                            crate::frontend::permission_action(op_dsc)
                        );
    ...
    let route_contract_path_arms_ts = crate::model::OpDsc::ALL.iter().map(|op_dsc| {
        let operation = quote::format_ident!("{}", op_dsc.op.to_string());
        let path = format!("/{}/{}", ident_sc_string, op_dsc.op.self_sc_str());
        quote::quote! {#ident_operation_ucc::#operation => #path}
    });
    let route_contract_operation_kind_arms_ts = crate::model::OpDsc::ALL.iter().map(|op_dsc| {
        let operation = quote::format_ident!("{}", op_dsc.op.to_string());
        let operation_kind = match crate::frontend::operation_kind(op_dsc) {
    ...
    let generated_contract_tests_ts = {
        ...
        let round_trip_tests_ts = crate::model::OpDsc::ALL.iter().map(|op_dsc| {
            let op = &op_dsc.op;
        ...
        let route_open_api_parity_assertions_ts = crate::model::OpDsc::ALL
            .iter()
            .filter(|op_dsc| op_is_enabled(&op_dsc.op))
            .map(|op_dsc| {
            let operation = op_dsc.op.self_sc_str();
            let path = format!("/{ident_sc_string}/{operation}");
            let method = match crate::contract_tests::http_method(op_dsc) {
                OpHttpMethod::Post => "post",
                OpHttpMethod::Patch => "patch",
                OpHttpMethod::Delete => "delete",
            };
            let success_status = if crate::contract_tests::success_status(op_dsc)
                == macros_helpers::status_code::StatusCode::Crd201
            {
                "201"
            } else {
