...
mod tests {
    ...
    ) -> super::super::model::OperationDsc<bool, (), (), (), (), ()> {
        super::super::model::OperationDsc {
            http_method: (),
            idempotency_capable,
            operation: (),
            operation_kind: (),
    ...
    fn capability_projection_returns_each_independent_descriptor_flag() {
        ...
        assert!(!super::optimistic_concurrency_capable(&idempotent));

        let optimistic = descriptor(false, true);
        assert!(!super::idempotency_capable(&optimistic));
        assert!(super::optimistic_concurrency_capable(&optimistic));
    }
}
